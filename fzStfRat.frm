...
Begin VB.Form frmRStaffRatio 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   Caption         =   "Staff/Management Ratios Report"
   ClientHeight    =   8715
   ClientLeft      =   570
   ClientTop       =   1095
   ClientWidth     =   10275
   ...
   Begin VB.Frame scrFrame 
      ...
      Begin VB.ComboBox cmbDateBased 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "fzStfRat.frx":0000
         Left            =   2220
         List            =   "fzStfRat.frx":0002
         Style           =   2  'Dropdown List
         TabIndex        =   14
      ...
      Begin INFOHR_Controls.CodeLookup clpCode 
         ...
         Left            =   1920
         TabIndex        =   16
         Tag             =   "01-Non-Supervisory Position Group - Code"
         Top             =   5040
         Width           =   7515
         _ExtentX        =   13256
         _ExtentY        =   503
      ...
      Begin INFOHR_Controls.CodeLookup clpCode 
         ...
         Left            =   1920
         TabIndex        =   17
         Tag             =   "01-Non-Management Position Group - Code"
         Top             =   5400
         Width           =   7515
         _ExtentX        =   13256
         _ExtentY        =   503
      ...
      Begin INFOHR_Controls.CodeLookup clpCode 
         ...
         Left            =   1920
         TabIndex        =   18
         Tag             =   "01-Supervisory Position Group - Code"
         Top             =   5760
         Width           =   7515
         _ExtentX        =   13256
         _ExtentY        =   503
      ...
      Begin INFOHR_Controls.CodeLookup clpCode 
         ...
         Left            =   1920
         TabIndex        =   19
         Tag             =   "01-Management Position Group - Code"
         Top             =   6120
         Width           =   7515
         _ExtentX        =   13256
         _ExtentY        =   503
   ...
   End
End
Attribute VB_Name = "frmRStaffRatio"
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "CRW", "HREMP_HS", "SELECT")
Resume Next
...
'Call Calculate_Staff_Ratios
Call SETWRK

If comGroup(0) = "(none)" Then
    strRName$ = glbIHRREPORTS & "rzStfRat.rpt"
Else
    strRName$ = glbIHRREPORTS & "rzStfRa1.rpt"
End If
...
Me.vbxCrystal.WindowTitle = "Staff/Management Ratios Report"
...
'    MsgBox "Non-Supervisory Group cannot be blank"
'    clpCode(7).SetFocus
'    Exit Function
'End If
...
'    MsgBox "Non-Management Group cannot be blank"
'    clpCode(8).SetFocus
'    Exit Function
'End If
...
'    MsgBox "Supervisory Group cannot be blank"
'    clpCode(9).SetFocus
'    Exit Function
'End If

If Not clpCode(10).ListChecker Then
    Exit Function
End If
If Len(Trim(clpCode(10).Text)) = 0 Then
    MsgBox "Management Group cannot be blank"
    clpCode(10).SetFocus
    Exit Function
End If
...
Private Sub Calculate_Staff_Ratios()
Dim ESQLQ, SQLQ, SQLQ1, SQLQ2, SQLQM, SQLQS, SQLQnMS, SQLQ3a, SQLQ3b, SQLQT
Dim rsEMP As New ADODB.Recordset
Dim TotEmp, Mngmnt, Superv, NonMngSprv
    ...
    SQLQ1 = "SELECT COUNT(ED_EMPNBR) AS EMP_COUNT FROM HREMP WHERE " & ESQLQ
    ...
    'Total Employees
    If Len(clpCode(3).Text) > 0 Then
        SQLQ2 = SQLQ2 & SQLQ & IIf(glbOracle, SQLQ3b, SQLQ3a)
    End If
    ...
    SQLQM = " AND ED_ORG IN ('" & getCodes(clpCode(10).Text) & "')"
    ...
    If Len(Trim(clpCode(10).Text)) > 0 Then
        'If Len(Trim(clpCode(3).Text)) = 0 Then
        '    SQLQM = SQLQM & getCodes(clpCode(10).Text)
        'Else
        '    SQLQM = SQLQM & ",'" & getCodes(clpCode(10).Text)
        'End If
        
        'SQLQM = SQLQM & IIf(glbOracle, SQLQ3b, SQLQ3a)
    Else
        'SQLQM = SQLQ1
    End If
    
    rsEMP.Open SQLQM, gdbAdoIhr001, adOpenKeyset
    If Not rsEMP.EOF Then
        Mngmnt = rsEMP("EMP_COUNT")
    Else
        Mngmnt = 0
    End If
    rsEMP.Close
    
    'Supervisors (Non-Management)
    SQLQS = " AND ED_ORG IN ('" & getCodes(clpCode(9).Text) & "')"
    ...
    If Len(Trim(clpCode(9).Text)) > 0 Then
        'If Len(Trim(clpCode(3).Text)) = 0 Then
        '    SQLQS = SQLQS & getCodes(clpCode(9).Text)
        'Else
        '    SQLQS = SQLQS & ",'" & getCodes(clpCode(9).Text)
        'End If
        
        'SQLQS = SQLQS & IIf(glbOracle, SQLQ3b, SQLQ3a)
    Else
        'SQLQS = SQLQ1
    End If
    
    rsEMP.Open SQLQS, gdbAdoIhr001, adOpenKeyset
    If Not rsEMP.EOF Then
        Superv = rsEMP("EMP_COUNT")
    Else
        Superv = 0
    End If
    rsEMP.Close
    ...
    'Non-Management and Non-Supervisor
    
    SQLQnMS = " AND ED_ORG IN ('" & getCodes(clpCode(7).Text) & "','" & getCodes(clpCode(7).Text) & "')"
    
    If Len(Trim(clpCode(3).Text)) = 0 Then
        'SQLQnMS = SQLQ1 & SQLQ
        SQLQnMS = SQLQ1 & SQLQnMS
    End If
    
    If Len(Trim(clpCode(7).Text)) > 0 Then
        'If Len(Trim(clpCode(3).Text)) = 0 Then
        '    SQLQnMS = SQLQnMS & getCodes(clpCode(7).Text)
        'Else
        '    SQLQnMS = SQLQnMS & ",'" & getCodes(clpCode(7).Text)
        'End If
        
        'If Len(Trim(clpCode(8).Text)) > 0 Then
        '    SQLQnMS = SQLQnMS & "','" & getCodes(clpCode(8).Text)
        'End If
        'SQLQnMS = SQLQnMS & IIf(glbOracle, SQLQ3b, SQLQ3a)
        
    ElseIf Len(Trim(clpCode(8).Text)) > 0 Then
        'If Len(Trim(clpCode(3).Text)) = 0 Then
        '    SQLQnMS = SQLQnMS & getCodes(clpCode(8).Text)
        'Else
        '    SQLQnMS = SQLQnMS & ",'" & getCodes(clpCode(8).Text)
    ...
    rsEMP.Open SQLQnMS, gdbAdoIhr001, adOpenKeyset
    If Not rsEMP.EOF Then
        NonMngSprv = rsEMP("EMP_COUNT")
    Else
        NonMngSprv = 0
    End If
    rsEMP.Close
    ...
    If chkTerm Then
        ...
        SQLQ1 = "SELECT COUNT(ED_EMPNBR) AS EMP_COUNT FROM Term_HREMP WHERE " & ESQLQ
        
        SQLQT = ""
        If IsDate(dlpDateRange(0)) And IsDate(dlpDateRange(1)) Then
            SQLQT = SQLQT & " AND TERM_SEQ IN (SELECT TERM_SEQ FROM Term_HRTRMEMP "
            SQLQT = SQLQT & in_SQL(glbIHRAUDIT)
            SQLQT = SQLQT & " WHERE Term_DOT >=" & Date_SQL(dlpDateRange(0))
            SQLQT = SQLQT & " AND Term_DOT <=" & Date_SQL(dlpDateRange(1)) & ")"
        Else
            If IsDate(dlpDateRange(0)) Then
                SQLQT = SQLQT & " AND TERM_SEQ IN (SELECT TERM_SEQ FROM Term_HRTRMEMP "
                SQLQT = SQLQT & in_SQL(glbIHRAUDIT)
            ...
            If IsDate(dlpDateRange(1)) Then
                SQLQT = SQLQT & " AND TERM_SEQ IN (SELECT TERM_SEQ FROM Term_HRTRMEMP "
                SQLQT = SQLQT & in_SQL(glbIHRAUDIT)
                SQLQT = SQLQT & " WHERE Term_DOT <=" & Date_SQL(dlpDateRange(1)) & ")"
            End If
        End If
        ...
        rsEMP.Open SQLQnMS, gdbAdoIhr001, adOpenKeyset
        If Not rsEMP.EOF Then
            NonMngSprv = NonMngSprv + rsEMP("EMP_COUNT")
        End If
        rsEMP.Close
    End If
    '-------------------------------------------------------------------------------
    
    Me.vbxCrystal.Formulas(1) = "Title='Staff/Management Ratios'"
    
    If Len(dlpDateRange(3).Text) > 0 Then
        Me.vbxCrystal.Formulas(2) = "AsOfDate='" & dlpDateRange(3).Text & "'"
    End If
    If chkTerm Then
        Me.vbxCrystal.Formulas(3) = "TotalDescEmployee='Total Number of Active and Terminated Employees: " & TotEmp & "'"
        Me.vbxCrystal.Formulas(4) = "TotalManagement='Total Active and Terminated Management: " & Mngmnt & "'"
        Me.vbxCrystal.Formulas(5) = "TotalSupervisor='Total Active and Terminated Supervisor: " & Superv & "'"
        Me.vbxCrystal.Formulas(6) = "TotalNonSuperManage='Total Active and Terminated Non-Management, Non-Supervisor: " & NonMngSprv & "'"
    Else
        Me.vbxCrystal.Formulas(3) = "TotalDescEmployee='Total Number of Employees: " & TotEmp & "'"
        Me.vbxCrystal.Formulas(4) = "TotalManagement='Total Management: " & Mngmnt & "'"
        Me.vbxCrystal.Formulas(5) = "TotalSupervisor='Total Supervisor: " & Superv & "'"
        Me.vbxCrystal.Formulas(6) = "TotalNonSuperManage='Total Non-Management, Non-Supervisor: " & NonMngSprv & "'"
    End If
...
Me.vbxCrystal.Formulas(2) = "Title='Staff/Management Ratios'"
...
'Manager codes
'If Len(Trim(clpCode(10).Text)) > 0 Then
'    If InStr(getCodes(clpCode(10).Text), ",") <> 0 Then
        dscGroup$ = Replace(getCodes(clpCode(10).Text), ",", " OR {HREMP.ED_ORG} = ")
        dscGroup$ = "Manager=if {HREMP.ED_ORG} = '" & dscGroup$ & "' then 1 else 0"
'    Else
'        dscGroup$ = "Manager=if {HREMP.ED_ORG} = ('" & getCodes(clpCode(10).Text) & "') then 1 else 0"
'    End If
...
'Supervisor codes
'If Len(Trim(clpCode(9).Text)) > 0 Then
'    If InStr(getCodes(clpCode(9).Text), ",") <> 0 Then
        dscGroup$ = Replace(getCodes(clpCode(9).Text), ",", " OR {HREMP.ED_ORG} = ")
        dscGroup$ = "Supervisor=if {HREMP.ED_ORG} = '" & dscGroup$ & "' then 1 else 0"
'    Else
'        dscGroup$ = "Supervisor=if {HREMP.ED_ORG} = ('" & getCodes(clpCode(9).Text) & "') then 1 else 0"
'    End If
    Me.vbxCrystal.Formulas(5) = dscGroup$
'End If

'Non-Supervisor/Non-Management codes
If Len(Trim(clpCode(7).Text)) > 0 Then
    'If InStr(getCodes(clpCode(7).Text), ",") <> 0 Then
        dscGroup$ = Replace(getCodes(clpCode(7).Text), ",", " OR {HREMP.ED_ORG} = ")
        dscGroup$ = "NonMgrSuperv=if {HREMP.ED_ORG} = '" & dscGroup$ & "'"
    'Else
    '    dscGroup$ = "NonMgrSuperv=if {HREMP.ED_ORG} = '" & getCodes(clpCode(7).Text) & "'"
    'End If
    
    If Len(Trim(clpCode(8).Text)) > 0 Then
        'If InStr(getCodes(clpCode(8).Text), ",") <> 0 Then
            dscGroup$ = dscGroup$ & " OR {HREMP.ED_ORG} = '" & Replace(getCodes(clpCode(8).Text), ",", " OR {HREMP.ED_ORG} = ")
            dscGroup$ = dscGroup$ & "'"
        'Else
        '    dscGroup$ = dscGroup$ & " OR {HREMP.ED_ORG} = '" & getCodes(clpCode(8).Text) & "'"
        'End If
    End If
    
    Me.vbxCrystal.Formulas(6) = dscGroup$ & " then 1 else 0"
Else
    'If Len(Trim(clpCode(8).Text)) > 0 Then
        'If InStr(getCodes(clpCode(8).Text), ",") <> 0 Then
            dscGroup$ = Replace(getCodes(clpCode(8).Text), ",", " OR {HREMP.ED_ORG} = ")
            dscGroup$ = "NonMgrSuperv=if {HREMP.ED_ORG} = '" & dscGroup$ & "' then 1 else 0"
        'Else
        '    dscGroup$ = "NonMgrSuperv=if {HREMP.ED_ORG} = ('" & getCodes(clpCode(8).Text) & "') then 1 else 0"
        'End If
