...
Begin VB.Form frmTermUEmpNum 
   Appearance      =   0  'Flat
   Caption         =   "Terminated Employee Number Mass Change"
   ClientHeight    =   5550
   ClientLeft      =   930
   ClientTop       =   1560
   ClientWidth     =   8565
   ...
   Begin VB.Frame frmGeneral 
      ...
      Begin VB.Label lblEmpNum 
         ...
         BackColor       =   &H80000005&
         BackStyle       =   0  'Transparent
         Caption         =   "NEW Terminated Employee #"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Label lblEmpNum 
      ...
      BackColor       =   &H80000005&
      BackStyle       =   0  'Transparent
      Caption         =   "OLD Terminated Employee #"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Label lblMessage 
      Caption         =   "Ensure that no other user is accessing this terminated employee's information during this procedure."
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   13.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmTermUEmpNum"
...
Unload Me
If glbOnTop = "FRMTERMUEMPNUM" Then glbOnTop = ""
End Sub
...
SQLQ = "Select ED_EMPNBR,ED_ORG_TABL,ED_ORG from Term_HREMP WHERE ED_EMPNBR = " & elpEmpNum(0).Text
...
Else
    ...
    End If
End If
Title$ = "Terminated Employee Number Change"
DgDef = MB_YESNO + MB_ICONSTOP + MB_DEFBUTTON2  ' Describe dialog.
Msg$ = "Are you sure you want to change the Employee number for this Terminated Employee?"
...
glbTERM_ID = getEmpnbr(txtEmpNum(1)) 'CHANGE THE ID NUMBER OF THE GLOBAL SELECTED EMPLOYEE
DoEvents
elpEmpNum(0).Text = ""
txtEmpNum(1) = ""
clpDIV = ""
...
If glbTERM_ID > 0 Then
    frmTermUEmpNum.Show
Else
    frmTERMEMPL.Show 1  ' find EE if not present
End If

If Len(glbTerm_SName) < 1 Then Exit Sub
Screen.MousePointer = HOURGLASS


If Len(glbTerm_SName) > 0 Then
    Me.elpEmpNum(0).Caption = RTrim$(glbTerm_SName) & ", " & RTrim$(glbTerm_FName)
End If
elpEmpNum(0).Text = ShowEmpnbr(glbTERM_ID)
elpEmpNum(0).LookupType = TERM
If glbCompSerial = "S/N - 2296W" Then
...
SQLQ = "SELECT * FROM INFO_HR_TABLES WHERE TERMINATION_TABLE=1"
...
Do Until dyn_Table.EOF
    ...
    If xTable = "Term_HRTRMEMP" Then xFldTitle = "Term_"
    If dyn_Table("Employee_Keyed") Then
        ...
        Case "TERM_HR_OCC_HEALTH_SAFETY"
...
'Ticket #25678 franks 07/09/2014
'Pension tables Ticket
If glbSQL Then
    If glbWFC Then
...
rsAU("AU_TYPE") = "T"
rsAU("AU_NEWEMP") = "N"
rsAU("AU_SURNAME") = glbTerm_SName
rsAU("AU_FNAME") = glbTerm_FName
...
'rsPerform.Close
xEMPNBR = txtEmpNum(1)
SQLQ = "SELECT ED_EMPNBR, ED_DIV FROM TERM_HREMP WHERE ED_EMPNBR = " & xEMPNBR
...
SQLQ = "SELECT * FROM TERM_HREMP WHERE ED_EMPNBR = " & xEMPNBR
rsEMP.Open SQLQ, gdbAdoIhr001, adOpenStatic
rsAU.AddNew
rsAU("AU_COMPNO") = "001"
rsAU("AU_LOC_TABL") = "EDLC": rsAU("AU_SECTION_TABL") = "EDSE": rsAU("AU_EMP_TABL") = "EDEM"
...
Set frmTermUEmpNum = Nothing 'carmen apr 2000
End Sub
...
If Len(elpEmpNum(0).Text) = 0 Then
  MsgBox "The Terminated Employee number must be entered"
  elpEmpNum(0).SetFocus
  Exit Function
End If
If elpEmpNum(0).Caption = "Enter Valid Employee #" Then  'the OLD employee number not valid
  MsgBox "The OLD Terminated Employee number is not valid"
  elpEmpNum(0).SetFocus
  Exit Function
End If
If getEmpnbr(elpEmpNum(0).Text) = glbEmpNbr Then
  MsgBox "You cannot change your own Terminated Employee number"
  elpEmpNum(0).SetFocus
  Exit Function
End If
If glbLinamar Then
    ...
    Else
        ...
        End If
    End If
    If lblEEName(1).Visible = True Then 'the employee number to change to already exixts
      MsgBox "The NEW Terminated Employee number already exists"
      txtEmpID.SetFocus
      Exit Function
    End If
Else
...
Private Sub UpdHRPensionTables(zEMPNBR, Optional zOldEmpnbr) 'Ticket #25678 franks 06/30/2014
On Error GoTo EndSub
Dim SQLQ
Dim xStr
    xStr = " AND PE_SIN IN (SELECT ED_SIN FROM TERM_HREMP) "
    
    SQLQ = "UPDATE HRP_DB_CONTRIBUTION SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_DB_CONTRIBUTION_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    
    SQLQ = "UPDATE HRP_PAR SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_PAR_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    ...
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_PA_DETAILS_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    ...
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_PA_MASTER_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    ...
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_PENSION_BENEFICIARY_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    ...
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_PENSION_MASTER_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    
    SQLQ = "UPDATE HRP_PENSION_MASTER_AUDIT SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_SPECIAL_EARLY_RET SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    
    SQLQ = "UPDATE HRP_SUNLIFE SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
    SQLQ = "UPDATE HRP_SUNLIFE_ARC SET PE_EMPNBR=" & zEMPNBR & " WHERE PE_EMPNBR = " & zOldEmpnbr
    SQLQ = SQLQ & xStr
    gdbAdoIhr001.Execute SQLQ
Exit Sub
EndSub:
