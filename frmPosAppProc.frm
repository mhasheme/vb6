...
Begin VB.Form frmPosAppProc 
   ...
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "frmPosAppProc.frx":0000
      Height          =   2115
      Left            =   120
      OleObjectBlob   =   "frmPosAppProc.frx":0014
      TabIndex        =   0
      Tag             =   "Skills Lookup"
      Top             =   600
      Width           =   9675
   ...
   End
End
Attribute VB_Name = "frmPosAppProc"
...
If Not gSec_Upd_AppProcess Then
...
Public Sub cmdPrint_Click()
    ...
    Me.vbxCrystal.SelectionFormula = "{HRJOB_APP_PROCESS.JA_JOB}='" & glbPos & "'"
    Me.vbxCrystal.Destination = crptToPrinter
    
    Me.vbxCrystal.WindowTitle = Me.Caption
    Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgPosProc.rpt"
...
Private Sub cmdPrintAll_Click()
 '   Me.vbxCrystal.SelectionFormula = glbstrSelCri
    ...
    Me.vbxCrystal.WindowTitle = "Application Process"
    Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgPosProc.rpt"
    Me.vbxCrystal.Connect = RptODBC_SQL
    Me.vbxCrystal.Action = 1
End Sub
...
Dim OLDJob, FromJob, SQLQ
Dim rsFrom As New ADODB.Recordset
Dim rsCurrent As ADODB.Recordset
    OLDJob = glbPos
    frmJOBS.Show 1
    FromJob = glbPos
    If FromJob <> OLDJob Then
        glbPos = OLDJob
        Set rsCurrent = Data1.Recordset.Clone
        SQLQ = "SELECT * FROM HRJOB_APP_PROCESS "
        SQLQ = SQLQ & " WHERE JA_JOB = '" & FromJob & "' "
        SQLQ = SQLQ & " ORDER BY JA_SEQ"
        rsFrom.Open SQLQ, gdbAdoIhr001, adOpenForwardOnly
        Do Until rsFrom.EOF
            rsCurrent.Filter = "JA_SEQ=" & rsFrom("JA_SEQ")
            If rsCurrent.EOF Then
                rsCurrent.AddNew
                rsCurrent("JA_JOB") = glbPos
                rsCurrent("JA_SEQ") = rsFrom("JA_SEQ")
                rsCurrent("JA_DESC") = rsFrom("JA_DESC")
                rsCurrent.Update
            End If
            rsFrom.MoveNext
        Loop
...
Me.Caption = "Application Process- " & lblPosition
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Application Process", "HRJOB_APP_PROCESS", "SELECT")
Call RollBack   '15June99 js
...
If IsNull(rsDATA("JA_ID")) Then
    PID& = 0
Else
    PID& = rsDATA("JA_ID") ' CLng(Val(lblID))
End If
xPosCtrl = txtSeq
...
SQLQ$ = "SELECT * FROM HRJOB_APP_PROCESS "
SQLQ$ = SQLQ$ & "WHERE "
SQLQ$ = SQLQ$ & " (JA_JOB = '" & Pos$ & "' "
SQLQ$ = SQLQ$ & "AND JA_SEQ = " & xPosCtrl
SQLQ$ = SQLQ$ & "AND JA_ID <> " & ID& & ") "
...
Public Sub Display_Value()
    ...
    SQLQ = "SELECT * FROM HRJOB_APP_PROCESS "
    SQLQ = SQLQ & "WHERE JA_ID = " & Data1.Recordset!JA_ID
...
UpdateRight = gSec_Upd_AppProcess
End Property
