...
Begin VB.Form frmESALARY2 
   ...
   Begin Threed.SSPanel panWindow 
      ...
      Begin VB.PictureBox panDetails 
         ...
         Begin Threed.SSFrame fraSalary 
            ...
            EndProperty
            Font3D          =   1
            Begin VB.ComboBox comSalScale1 
               Enabled         =   0   'False
               Height          =   315
               Left            =   5820
               TabIndex        =   123
            ...
            Begin INFOHR_Controls.DateLookup dlpDate 
               ...
               TextBoxWidth    =   1215
            End
            Begin MSMask.MaskEdBox medsalary1 
   ...
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "Fesalary2.frx":0000
      Height          =   1455
      Left            =   0
      OleObjectBlob   =   "Fesalary2.frx":0014
      TabIndex        =   0
      Top             =   600
      Width           =   11295
   End
End
Attribute VB_Name = "frmESALARY2"
...
Dim orgIndSalary As Double
Dim orgIndSalary1 As Double
Dim OSalary, OSalCD, oJob, OEDate, ONDate, OReason, ONFSalary, OSalary1
...
If IsNull(OSalary) Then
    OSalary = 0
End If
'Ticket #30452 - ONE CARE
If IsNull(OSalary1) Then
    OSalary1 = 0
End If
...
Call NextForm
Unload Me
If glbOnTop = "FRMESALARY2" Then glbOnTop = ""
...
If glbFrench Then
    ...
    'Ticket #30452 - ONE CARE
    If IsNumeric(medsalary1.Text) Then
        orgIndSalary1 = medsalary1
        orgIndSalary = medsalary1
    End If
Else
    ...
    'Ticket #30452 - ONE CARE
    orgIndSalary1 = Val(medsalary1)
    orgIndSalary = Val(medsalary1)
End If
...
If gsEMAIL_ONSALARY Then
    ...
    If NewHireForms.count = 0 Then 'Non new hire
        If OSalary <> medsalary And OSalary > 0 And (fglbNew Or chkCurrent) And OSalary > 0 Then  'Only Salary Change
            ...
            'Ticket #30452 - ONE CARE
            If OSalary1 <> medsalary1 And OSalary1 > 0 And OSalary1 > 0 Then
                MailBody = MailBody & vbCrLf
                MailBody = MailBody & "The Indirect Salary has been changed." & vbCrLf & vbCrLf
                MailBody = MailBody & "Employee #: " & lblEENum.Caption & vbCrLf
                MailBody = MailBody & "Name: " & lblEEName.Caption & vbCrLf
                MailBody = MailBody & "New Indirect Salary: " & (Format(medsalary1, "$#.00")) & "/" & comPayPer.Text & vbCrLf
                MailBody = MailBody & "Reason: " & GetTABLDesc("SDRC", clpCode(1)) & vbCrLf
                MailBody = MailBody & "Effective Date: " & dlpDate(0) & vbCrLf
            End If
            ...
            'Call imgEmail_Click
        ElseIf OSalary1 <> medsalary1 And OSalary1 > 0 And (fglbNew Or chkCurrent) And OSalary1 > 0 Then    'Ticket #30452 - ONE CARE
            MailBody = "The Indirect Salary has been changed." & vbCrLf & vbCrLf
            MailBody = MailBody & "Employee #: " & lblEENum.Caption & vbCrLf
            MailBody = MailBody & "Name: " & lblEEName.Caption & vbCrLf
            MailBody = MailBody & "New Indirect Salary: " & (Format(medsalary1, "$#.00")) & "/" & comPayPer.Text & vbCrLf
            MailBody = MailBody & "Reason: " & GetTABLDesc("SDRC", clpCode(1)) & vbCrLf
...
Else
    ...
    If IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) And IsNumeric(medsalary1.Text) Then         'Ticket #30452 - ONE CARE
        medTotal.Text = CDbl(medsalary.Text) + CDbl(medPremium.Text) + CDbl(medsalary1.Text)
    ElseIf IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) = False And IsNumeric(medsalary1.Text) Then    'Ticket #30452 - ONE CARE
        medTotal.Text = medsalary.Text + medsalary1.Text
    ElseIf IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) = False And IsNumeric(medsalary1.Text) = False Then     'Ticket #30452 - ONE CARE
        medTotal.Text = medsalary.Text
    ElseIf IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) And IsNumeric(medsalary1.Text) = False Then          'Ticket #30452 - ONE CARE
...
If glbWFC Then 'Ticket #25927 Franks 08/26/2014 - for hrsoft missing position process
    'If NewHireForms.count = 0 Then
        If glbCandidate > 0 Then
             Call WFCHRSoftProcUpt("frmESALARY2")
        End If
    'End If
End If
...
ElseIf glbCompSerial = "S/N - 2437W" Then 'KN&V Ticket #21097 Franks 11/02/2011
    ...
    Call ScreenKNV
Else
    If glbCompSerial = "S/N - 2494W" Then   'Ticket #30452 - ONE CARE
        'Do not change the height or top of the Salary frame.
    Else
        fraSalary.Height = 555
...
'End If
'Ticket #30452 - OneCare
comPayPer1.Clear
comPayPer1.AddItem "Annum"
comPayPer1.AddItem "Hour "
comPayPer1.AddItem "Monthly "
comPayPer1.AddItem "Daily "
...
If Len(lblSalCode.Caption) > 0 Then
    ...
    ElseIf lblSalCode.Caption = "D" Then  'Ticket #17654
        ...
        'Ticket #30452 - OneCare
        comPayPer1.ListIndex = 3
    Else
        comPayPer.ListIndex = 2
        'Ticket #30452 - OneCare
        comPayPer1.ListIndex = 2
    End If
End If
End Sub
Sub Set_WFC_COMPA()
...
'Ticket #30452 - ONE CARE
If Not IsNumeric(medsalary1) Then medsalary1 = 0

If glbFrench Then
    medsalary = Round2DEC(medsalary)    'Val() causing the values to trunc to 0 decimal places
    'Ticket #30452 - ONE CARE
    medsalary1 = Round2DEC(medsalary1)
Else
    medsalary = Round2DEC(Val(medsalary))
    'Ticket #30452 - ONE CARE
    medsalary1 = Round2DEC(Val(medsalary1))
End If
        ...
        If IsNumeric(medsalary) Then
            ...
            If fglbPhrs <> 0 Then
                ...
                If lblSalCode = "H" Then
                    ...
                    Else
                        lblPayPeriodSalary = Round2DEC((Val(medsalary) + Val(medsalary1)) * fglbPhrs) 'Ticket #30452 - ONE CARE
                    End If
                    ...
                    'If glbCompSerial = "S/N - 2172W" Then 'County of Lanark Ticket #17076
                        ...
                        Else
                            ...
                            Else
                                lblHoursPay.Caption = Round2DEC((Val(medsalary) + Val(medsalary1)) * fglbWhrs# * 52)     'Ticket #30452 - ONE CARE
                            End If
                ...
                ElseIf lblSalCode = "M" Then
                    ...
                    lblPayPeriodSalary.Caption = Round2DEC((Val(medsalary) + Val(medsalary1)) * 12)       'Ticket #30452 - ONE CARE
                    ...
                    If fglbWhrs# <> 0 Then
                        ...
                        If glbFrench Then
                            ...
                            lblHoursPay = Round2DEC((((Val(medsalary) + Val(medsalary1)) * 12) / 52) / (fglbWhrs#))      'Ticket #30452 - ONE CARE
                        Else
                            'Ticket #20396 - Jerry said this formula is not correct so gave the correct one
                ...
                ElseIf lblSalCode = "D" Then
                    ...
                    'If IsDate(dlpDate(0)) Then
                        ...
                        lblPayPeriodSalary.Visible = True
                        lblTitle(15).Visible = True
                        lblPayPeriodSalary.Caption = Round2DEC(((Val(medsalary) + Val(medsalary1)) * 5) * 52)  'Assumes 5 day/week        'Ticket #30452 - ONE CARE
                        lblPayPeriodSalary.Caption = Format(lblPayPeriodSalary, "#0." & String(glbCompDecHR, "0"))
                        lblTitle(15).Caption = "Annual Salary : "
                        ...
                        If fglbDhrs <> 0 Then
                            If glbFrench Then
                                lblHoursPay = Round2DEC((Val(medsalary) + Val(medsalary1)) / fglbDhrs)       'Ticket #30452 - ONE CARE
                            Else
                                lblHoursPay = Round2DEC((Val(medsalary) + Val(medsalary1)) / fglbDhrs)  'Ticket #30452 - ONE CARE
                            End If
                        Else
                ...
                Else
                    ...
                    Else
                        lblPayPeriodSalary = Round2DEC((((Val(medsalary) + Val(medsalary1)) / 52) / fglbWhrs#) * fglbPhrs)      'Ticket #30452 - ONE CARE
                    End If
                    ...
                    Else
                        ...
                        If Val(txtWHRS.Text) <> 0 Then 'Ticket #13210
                            If glbFrench Then
                                lblHoursPay = Round2DEC(((Val(medsalary) + Val(medsalary1)) / 52) / Val(txtWHRS.Text))      'Ticket #30452 - ONE CARE
                            Else
                                lblHoursPay = Round2DEC(((Val(medsalary) + Val(medsalary1)) / 52) / Val(txtWHRS.Text)) 'Ticket #30452 - ONE CARE
                            End If
                        Else
...
'Ticket #30452 - ONE CARE
Private Sub medSalary1_LostFocus()
Dim X%

On Error GoTo Salary1_Err

If Not IsNumeric(medsalary1) Then medsalary1 = 0

If glbFrench Then
    medsalary1 = Round2DEC(medsalary1)    'Val() causing the values to trunc to 0 decimal places
Else
    medsalary1 = Round2DEC(Val(medsalary1))
End If
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "medsalary1", "HR_SALARY_HISTORY", "Update")
Resume Next
Unload Me
...
If glbFrench Then
    ...
    Else
        If IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) And IsNumeric(medsalary1.Text) Then    'Ticket #30452 - ONE CARE
            medTotal.Text = CDbl(medsalary.Text) + CDbl(medPremium.Text) + CDbl(medsalary.Text)
        ElseIf IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) = False And IsNumeric(medsalary1.Text) Then     'Ticket #30452 - ONE CARE
            medTotal.Text = medsalary.Text + medsalary1.Text
        ElseIf IsNumeric(medsalary.Text) And IsNumeric(medPremium.Text) = False And IsNumeric(medsalary1.Text) = False Then       'Ticket #30452 - ONE CARE
            medTotal.Text = medsalary.Text
...
If glbCompDecHR = 4 Then
    ...
    'Ticket #30452 - ONE CARE
    medsalary1.Format = "#,##0.0000;(#,##0.0000)"
...
On Error GoTo JS_Err
SQLQ = "Select SH_SALARY,SH_SALARY1 from HR_SALARY_HISTORY"
SQLQ = SQLQ & " WHERE SH_EMPNBR = " & glbLEE_ID & " "
SQLQ = SQLQ & " AND SH_JOB = '" & clpPostCode.Text & "' "
'Hemu
...
Else
    ...
    'Ticket #30452 - ONE CARE
    orgIndSalary = HRSH_Snap("SH_SALARY1")
    orgIndSalary1 = HRSH_Snap("SH_SALARY1")
End If
...
Else
    ...
    If rsDATA.EOF Or rsDATA.BOF Then
        ...
        'were open
        If flgloaded Then
            If UCase(MDIMain.ActiveForm.name) = "FRMESALARY2" Then
                'Hemu
                Call SET_UP_MODE
            End If
        End If
...
If Len(glbLEE_SName) > 0 And Len(glbLEE_SName) > 0 Then
    frmESALARY2.Caption = "Salary - " & Left$(glbLEE_SName, 5)
    frmESALARY2.lblEEName = RTrim$(glbLEE_SName) & ", " & RTrim$(glbLEE_FName)
End If
If glbtermopen Then lblEEID = glbTERM_ID Else lblEEID = glbLEE_ID
'lblEEID = glbLEE_ID
