...
Begin VB.Form frmSDocType 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   BorderStyle     =   0  'None
   Caption         =   "Document Type Codes Security"
   ClientHeight    =   5955
   ClientLeft      =   465
   ClientTop       =   1410
   ClientWidth     =   9765
   ...
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "fsDocType.frx":0000
      Height          =   3285
      Left            =   240
      OleObjectBlob   =   "fsDocType.frx":0014
      TabIndex        =   0
      Top             =   480
      Width           =   9330
   End
...
glbErrNum& = Err
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Can Error", "HR_SECURE_DOCUMENT_TYPE", "Cancel")
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        ...
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_DOCUMENT_TYPE SET ACCESSABLE=1,MAINTAINABLE=1 WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        gdbAdoIhr001.BeginTrans
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_DOCUMENT_TYPE SET ACCESSABLE=1 WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
        gdbAdoIhr001.CommitTrans
    End If
...
glbFrmCaption$ = Me.Caption
glbErrNum& = Err
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "cmdEdit", "HR_SECURE_DOCUMENT_TYPE", "Add")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "cmdUpdate", "HR_SECURE_DOCUMENT_TYPE", "Update")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
...
Me.vbxCrystal.WindowTitle = "Document Type Codes Security Report"
...
Else
    ...
    Next
End If
Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgDocTypeSec.rpt"

If xTemplate = "" Or xTemplate = "TEMPLATE" Then
    Me.vbxCrystal.SelectionFormula = "{HR_SECURE_DOCUMENT_TYPE.USERID}='" & Replace(lblUSERID, "'", "''") & "' "
Else
    '????Ticket #24808 -  Retrieve template's security profile
    Me.vbxCrystal.SelectionFormula = "{HR_SECURE_DOCUMENT_TYPE.USERID}='" & Replace(xTemplate, "'", "''") & "' "
End If
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        gdbAdoIhr001.BeginTrans
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_DOCUMENT_TYPE SET ACCESSABLE=0,MAINTAINABLE=0 WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
        gdbAdoIhr001.CommitTrans
    End If
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "DATA1.error", "HR_SECURE_DOCUMENT_TYPE", "SELECT")
...
Me.Caption = lStr("Document Type Codes Security - ") & lblEEName
...
SQLQ = "SELECT * FROM HR_SECURE_DOCUMENT_TYPE "
If xTemplate = "" Or xTemplate = "TEMPLATE" Then
...
rsTD.Open "SELECT * FROM HRTABL WHERE TB_NAME='DOCT'", gdbAdoIhr001, adOpenStatic
Do Until rsTD.EOF
    If xTemplate = "" Or xTemplate = "TEMPLATE" Then
        rsSR.Open "SELECT * FROM HR_SECURE_DOCUMENT_TYPE WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME='" & rsTD("TB_KEY") & "' AND CODENAME IS NOT NULL", gdbAdoIhr001, adOpenStatic, adLockOptimistic
    Else
        '????Ticket #24808 -  Retrieve template's security profile
        rsSR.Open "SELECT * FROM HR_SECURE_DOCUMENT_TYPE WHERE USERID='" & Replace(xTemplate, "'", "''") & "' AND CODENAME='" & rsTD("TB_KEY") & "' AND CODENAME IS NOT NULL", gdbAdoIhr001, adOpenStatic, adLockOptimistic
    End If
    If rsSR.EOF Then
...
Private Sub Update_Users_withthis_Template(xTemplate)
    ...
    Do While Not rsSecBasic.EOF
        If Not IsNull(rsSecBasic("USERID")) Then
            ...
            Call SpecificFunction_Template_Based_Security_Profile_Update(rsSecBasic("USERID"), xTemplate, "Change", "DOCUMENTTYPE")
        End If
        rsSecBasic.MoveNext
    Loop
    rsSecBasic.Close
