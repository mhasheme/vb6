...
Begin VB.Form frmVacPerctg 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   Caption         =   "Vacation % Increase Master"
   ClientHeight    =   10950
   ClientLeft      =   2565
   ClientTop       =   525
   ClientWidth     =   11760
   ...
   Begin VB.Frame VacFram03 
      ...
      Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
         Bindings        =   "fsVacPerctg.frx":0000
         Height          =   1695
         Left            =   0
         OleObjectBlob   =   "fsVacPerctg.frx":0014
         TabIndex        =   132
         Top             =   0
         Width           =   9135
      End
   ...
   End
End
Attribute VB_Name = "frmVacPerctg"
...
glbErrNum& = Err
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "chkMUEntitle", "HR_VACATION_INCR", "edit/Add")
...
Sub cmdDelete_Click()
    ...
    Msg = Msg & Chr(10) & "The Vacation Increase Rules?  "
    
    A% = MsgBox(Msg, 36, "Confirm Delete")
    If A% <> 6 Then Exit Sub
    
    Call getWSQLQ("C")
    SQLQ = "DELETE FROM HR_VACATION_INCR WHERE " & fglbVSQLQ
...
Sub cmdOK_Click()
    ...
    Else
        Call getWSQLQ("C")
        SQLQ = "SELECT * FROM HR_VACATION_INCR WHERE " & fglbVSQLQ
    ...
    End If
    gdbAdoIhr001.BeginTrans
    SQLQ = "SELECT * FROM HR_VACATION_INCR"
    rsVE.Open SQLQ, gdbAdoIhr001, adOpenStatic, adLockOptimistic
    For x% = 0 To 24
        If Len(medLTServ(x%)) > 0 Then
            rsVE.AddNew
            rsVE("VC_ORDER") = x + 1
            rsVE("VC_ORG_TABL") = "EDOR"
            rsVE("VC_ORG") = clpCode(0).Text
            rsVE("VC_PT") = clpPT.Text
            rsVE("VC_DIV") = clpDiv.Text
            rsVE("VC_DEPT") = clpDept.Text
            rsVE("VC_EMP_TABL") = "EDEM"
            rsVE("VC_EMP") = clpCode(1).Text
            rsVE("VC_SECTION") = clpCode(3).Text
            rsVE("VC_LOC") = clpCode(4).Text
            'rsVE("VE_EDATE") = dlpAsOf.Text
            
            'Ticket #15276 - Commented
'            If Len(dlpDateRange(0).Text) > 0 Then
'                rsVE("VC_FRDATE") = dlpDateRange(0).Text
'            End If
'            If Len(dlpDateRange(1).Text) > 0 Then
'                rsVE("VC_TODATE") = dlpDateRange(1).Text
'            End If
            
            rsVE("VC_GRPCD_TABL") = "JBGC"
            rsVE("VC_GRPCD") = clpCode(2).Text
            rsVE("VC_FROM_HRS") = medLTServ(x%)
            rsVE("VC_TO_HRS") = medGTServ(x%)
            If medEntitle(x%) = "" Then
                rsVE("VC_PCT") = Null
            Else
                rsVE("VC_PCT") = medEntitle(x%)
            End If
...
Sub cmdPrint_Click()
    ...
    Me.vbxCrystal.Reset
    Me.vbxCrystal.WindowTitle = "Vacation % Increase Report"
    ...
    Else
        ...
        Next
    End If
    Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgvacincrmst.rpt"
    
    SQLQ = "(1=1) "
    If Len(clpDiv.Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_DIV} = '" & clpDiv.Text & "'"
    If Len(clpDept.Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_DEPT} = '" & clpDept.Text & "'"
    If Len(clpCode(0).Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_ORG} = '" & clpCode(0).Text & "'"
'    If Len(dlpAsOf.Text) > 0 Then
...
'    End If
    If Len(clpCode(1).Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_EMP} = '" & clpCode(1).Text & "'"
    If Len(clpPT.Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_PT} = '" & clpPT.Text & "' "
    If Len(clpCode(2).Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_GRPCD} = '" & clpCode(2).Text & "'"
    If Len(clpCode(3).Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_SECTION} = '" & clpCode(3).Text & "'"
    If Len(clpCode(4).Text) > 0 Then SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_LOC} = '" & clpCode(4).Text & "'"
...
'        SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_FRDATE}  = Date(" & dtYYY% & ", " & dtMM% & ", " & dtDD% & ") "
'    End If
'    If Len(dlpDateRange(1).Text) > 0 Then
'        dtYYY% = Year(dlpDateRange(1).Text)
'        dtMM% = Month(dlpDateRange(1).Text)
'        dtDD% = Day(dlpDateRange(1).Text)
'        SQLQ = SQLQ & " AND {HR_VACATION_INCR.VC_TODATE}  = Date(" & dtYYY% & ", " & dtMM% & ", " & dtDD% & ") "
'    End If
...
Private Sub Form_Load()
    ...
    SQLQ = "SELECT DISTINCT VC_DIV,VC_DEPT,VC_ORG,VC_LOC,VC_SECTION,VC_EMP,VC_PT,VC_GRPCD,VC_FRDATE,VC_TODATE FROM HR_VACATION_INCR "
    
    If glbDIVCount = 1 And glbLinamar Then
        SQLQ = SQLQ & " WHERE VC_DIV IN (select DIV from HR_DIVISION WHERE " & glbSeleDiv & ")"
    End If
...
Sub Display_Value()
    ...
    If Not Data1.Recordset.EOF Then
        SQLQ = "SELECT * FROM HR_VACATION_INCR "
        If IsNull(Data1.Recordset("VC_DIV")) Then
            SQLQ = SQLQ & " WHERE VC_DIV IS NULL"
        Else
            SQLQ = SQLQ & " WHERE VC_DIV = '" & Data1.Recordset("VC_DIV") & "'"
        End If
        If IsNull(Data1.Recordset("VC_DEPT")) Then
            SQLQ = SQLQ & " AND VC_DEPT IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_DEPT = '" & Data1.Recordset("VC_DEPT") & "'"
        End If
        If IsNull(Data1.Recordset("VC_ORG")) Then
            SQLQ = SQLQ & " AND VC_ORG IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_ORG = '" & Data1.Recordset("VC_ORG") & "'"
        End If
        If IsNull(Data1.Recordset("VC_LOC")) Then
            SQLQ = SQLQ & " AND VC_LOC IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_LOC = '" & Data1.Recordset("VC_LOC") & "'"
        End If
        If IsNull(Data1.Recordset("VC_SECTION")) Then
            SQLQ = SQLQ & " AND VC_SECTION IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_SECTION = '" & Data1.Recordset("VC_SECTION") & "'"
        End If
'        If Not IsNull(Data1.Recordset("VC_EDATE")) Then
'            SQLQ = SQLQ & " AND VC_EDATE = " & Date_SQL(Data1.Recordset("VC_EDATE"))
'        End If
        If IsNull(Data1.Recordset("VC_EMP")) Then
            SQLQ = SQLQ & " AND VC_EMP IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_EMP = '" & Data1.Recordset("VC_EMP") & "'"
        End If
        If IsNull(Data1.Recordset("VC_PT")) Then
            SQLQ = SQLQ & " AND VC_PT IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_PT = '" & Data1.Recordset("VC_PT") & "' "
        End If
        If IsNull(Data1.Recordset("VC_GRPCD")) Then
            SQLQ = SQLQ & " AND VC_GRPCD IS NULL"
        Else
            SQLQ = SQLQ & " AND VC_GRPCD = '" & Data1.Recordset("VC_GRPCD") & "'"
        End If
        
        If Not IsNull(Data1.Recordset("VC_FRDATE")) Then
            SQLQ = SQLQ & " AND VC_FRDATE = " & Date_SQL(Data1.Recordset("VC_FRDATE"))
        End If
        If Not IsNull(Data1.Recordset("VC_TODATE")) Then
            SQLQ = SQLQ & " AND VC_TODATE = " & Date_SQL(Data1.Recordset("VC_TODATE"))
        End If
        
        SQLQ = SQLQ & " Order By VC_DIV,VC_DEPT,VC_ORG,VC_EMP,VC_PT,VC_LOC,VC_SECTION,VC_ORDER "
        rsVE.Open SQLQ, gdbAdoIhr001, adOpenKeyset
        If Not IsNull(Data1.Recordset("VC_DIV")) Then clpDiv.Text = Data1.Recordset("VC_DIV")
        If Not IsNull(Data1.Recordset("VC_DEPT")) Then clpDept.Text = Data1.Recordset("VC_DEPT")
        If Not IsNull(Data1.Recordset("VC_ORG")) Then clpCode(0).Text = Data1.Recordset("VC_ORG")
        'If Not IsNull(Data1.Recordset("VC_EDATE")) Then dlpAsOf.Text = Data1.Recordset("VC_EDATE")
        If Not IsNull(Data1.Recordset("VC_EMP")) Then clpCode(1).Text = Data1.Recordset("VC_EMP")
        If Not IsNull(Data1.Recordset("VC_PT")) Then clpPT.Text = Data1.Recordset("VC_PT")
        If Not IsNull(Data1.Recordset("VC_GRPCD")) Then clpCode(2).Text = Data1.Recordset("VC_GRPCD")
        If Not IsNull(Data1.Recordset("VC_LOC")) Then clpCode(4).Text = Data1.Recordset("VC_LOC")
        If Not IsNull(Data1.Recordset("VC_SECTION")) Then clpCode(3).Text = Data1.Recordset("VC_SECTION")
        
        'Ticket #15276 - Commented
'        If Not IsNull(Data1.Recordset("VC_FRDATE")) Then dlpDateRange(0).Text = Data1.Recordset("VC_FRDATE")
'        If Not IsNull(Data1.Recordset("VC_TODATE")) Then dlpDateRange(1).Text = Data1.Recordset("VC_TODATE")

        'If Not IsNull(Data1.Recordset("VC_MANUAL")) Then chkManual.Value = Data1.Recordset("VC_MANUAL")
        
        Do While Not rsVE.EOF
            xOrder = rsVE("VC_ORDER")
            nOrder = Format(Val(xOrder), "##0") - 1
            If Not (nOrder < 0 Or nOrder > 24) Then
                If Not IsNull(rsVE("VC_FROM_HRS")) Then medLTServ(nOrder) = rsVE("VC_FROM_HRS")
                If Not IsNull(rsVE("VC_TO_HRS")) Then medGTServ(nOrder) = rsVE("VC_TO_HRS")
                If Not IsNull(rsVE("VC_PCT")) Then medEntitle(nOrder) = rsVE("VC_PCT")
...
If Len(xDiv) = 0 Then
    fglbVSQLQ = " (VC_DIV IS NULL OR VC_DIV='')"
Else
    fglbVSQLQ = "VC_DIV = '" & xDiv & "'"
End If
If Len(xDept) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_DEPT IS NULL OR VC_DEPT='')"
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_DEPT = '" & xDept & "'"
End If
If Len(xORG) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_ORG IS NULL OR VC_ORG='') "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_ORG = '" & xORG & "'"
End If
'If UCase(glbCompEntSick$) = "A" Then
'    If Len(xAsOf) > 0 Then fglbVSQLQ = fglbVSQLQ & " AND  VE_EDATE = " & Date_SQL(xAsOf)
'End If
If Len(xEMP) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_EMP IS NULL OR VC_EMP='')"
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_EMP = '" & xEMP & "'"
End If
If Len(xEmpMode) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_PT IS NULL OR VC_PT='')"
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_PT = '" & xEmpMode & "' "
End If
If Len(xGRPCE) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_GRPCD IS NULL OR VC_GRPCD='') "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_GRPCD = '" & xGRPCE & "'"
End If

If Len(xLoc) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_LOC IS NULL OR VC_LOC='') "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_LOC = '" & xLoc & "'"
End If
If Len(xSection) = 0 Then
    fglbVSQLQ = fglbVSQLQ & " AND (VC_SECTION IS NULL OR VC_SECTION='') "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_SECTION = '" & xSection & "'"
End If

'Sam 02/03/2006
If Not IsDate(xFromDate) Then
    fglbVSQLQ = fglbVSQLQ & " AND VC_FRDATE IS NULL  "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_FRDATE = " & Date_SQL(xFromDate)
End If
If Not IsDate(xToDate) Then
    fglbVSQLQ = fglbVSQLQ & " AND VC_TODATE IS NULL  "
Else
    fglbVSQLQ = fglbVSQLQ & " AND VC_TODATE = " & Date_SQL(xToDate)
End If
'Sam 02/03/2006
End Sub
