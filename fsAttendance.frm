...
Begin VB.Form frmSAttendance 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   BorderStyle     =   0  'None
   Caption         =   "Attendance Reason Codes Security"
   ClientHeight    =   5955
   ClientLeft      =   465
   ClientTop       =   1410
   ClientWidth     =   9765
   ...
   Begin Threed.SSPanel panControls 
      ...
      Begin VB.CommandButton cmdGrantInquire 
         Appearance      =   0  'Flat
         Caption         =   "Grant All &Show"
         Height          =   360
         Left            =   6420
         TabIndex        =   16
         Tag             =   "Grant All Inquire"
         Top             =   195
         Width           =   1545
      End
      Begin VB.CommandButton cmdPrint 
         ...
         Left            =   3480
         TabIndex        =   7
         Tag             =   "Print Follow Up Codes Security"
         Top             =   180
         Width           =   855
      End
   ...
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "fsAttendance.frx":0000
      Height          =   3285
      Left            =   240
      OleObjectBlob   =   "fsAttendance.frx":0014
      TabIndex        =   0
      Top             =   480
      Width           =   9330
   End
   ...
   End
End
Attribute VB_Name = "frmSAttendance"
...
glbErrNum& = Err
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Can Error", "HR_SECURE_ATTENDANCE", "Cancel")
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        ...
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_ATTENDANCE SET ACCESSABLE=1,MAINTAINABLE=1, LDATE = " & Date & ", LTIME = '" & Time$ & "', LUSER = '" & Replace(glbUserID, "'", "''") & "' WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
...
Msg = "Would you like to grant all Show to all codes?"
Title$ = "Grant all Show?"   ' zzz
DgDef = MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON3  ' Describe dialog.
Response% = MsgBox(Msg, DgDef, Title)    ' Get user response.
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        gdbAdoIhr001.BeginTrans
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_ATTENDANCE SET ACCESSABLE=1, LDATE = " & Date & ", LTIME = '" & Time$ & "', LUSER = '" & Replace(glbUserID, "'", "''") & "' WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
        gdbAdoIhr001.CommitTrans
    End If
...
glbFrmCaption$ = Me.Caption
glbErrNum& = Err
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "cmdEdit", "HR_SECURE_ATTENDANCE", "Add")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
...
If xTemplate = "TEMPLATE" Or xTemplate = "" Then
    ...
    Data1.Recordset("LDATE") = Date
    Data1.Recordset("LTIME") = Time$
    Data1.Recordset("LUSER") = glbUserID
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "cmdUpdate", "HR_SECURE_ATTENDANCE", "Update")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
...
Me.vbxCrystal.WindowTitle = lStr("Attendance Reason Codes Security Report")
...
Else
    ...
    Next
End If
Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgAttendSec.rpt"

If xTemplate = "" Or xTemplate = "TEMPLATE" Then
    Me.vbxCrystal.SelectionFormula = "{HR_SECURE_ATTENDANCE.USERID}='" & Replace(lblUSERID, "'", "''") & "' "
Else
    '????Ticket #24808 -  Retrieve template's security profile
    Me.vbxCrystal.SelectionFormula = "{HR_SECURE_ATTENDANCE.USERID}='" & Replace(xTemplate, "'", "''") & "' "
End If
...
If Response = IDYES Then
    ...
    If xTemplate = "TEMPLATE" Or xTemplate = "" Then
        gdbAdoIhr001.BeginTrans
        gdbAdoIhr001.Execute "UPDATE HR_SECURE_ATTENDANCE SET ACCESSABLE=0,MAINTAINABLE=0, LDATE = " & Date & ", LTIME = '" & Time$ & "', LUSER = '" & Replace(glbUserID, "'", "''") & "' WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME IS NOT NULL"
        gdbAdoIhr001.CommitTrans
    End If
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "DATA1.error", "HR_SECURE_ATTENDANCE", "SELECT")
...
Me.Caption = lStr("Attendance Reason Codes Security - ") & lblEEName
...
SQLQ = "SELECT * FROM HR_SECURE_ATTENDANCE "
If xTemplate = "" Or xTemplate = "TEMPLATE" Then
...
rsTD.Open "SELECT * FROM HRTABL WHERE TB_NAME='ADRE'", gdbAdoIhr001, adOpenStatic
Do Until rsTD.EOF
    If xTemplate = "" Or xTemplate = "TEMPLATE" Then
        rsSR.Open "SELECT * FROM HR_SECURE_ATTENDANCE WHERE USERID='" & Replace(glbSecUSERID, "'", "''") & "' AND CODENAME='" & rsTD("TB_KEY") & "' AND CODENAME IS NOT NULL", gdbAdoIhr001, adOpenStatic, adLockOptimistic
    Else
        rsSR.Open "SELECT * FROM HR_SECURE_ATTENDANCE WHERE USERID='" & Replace(xTemplate, "'", "''") & "' AND CODENAME='" & rsTD("TB_KEY") & "' AND CODENAME IS NOT NULL", gdbAdoIhr001, adOpenStatic, adLockOptimistic
    End If
    If rsSR.EOF Then
...
Private Sub Update_Users_withthis_Template(xTemplate)
    ...
    Do While Not rsSecBasic.EOF
        If Not IsNull(rsSecBasic("USERID")) Then
            ...
            Call SpecificFunction_Template_Based_Security_Profile_Update(rsSecBasic("USERID"), xTemplate, "Change", "ATTENDANCE")
        End If
        rsSecBasic.MoveNext
    Loop
    rsSecBasic.Close
