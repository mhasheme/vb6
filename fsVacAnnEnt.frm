...
Begin VB.Form frmSVacAnnEnt 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   Caption         =   "Annual Entitlement Master"
   ClientHeight    =   8490
   ClientLeft      =   2565
   ClientTop       =   525
   ClientWidth     =   11400
   ...
   Begin VB.Frame VacFram03 
      ...
      Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
         Bindings        =   "fsVacAnnEnt.frx":0000
         Height          =   1335
         Left            =   0
         OleObjectBlob   =   "fsVacAnnEnt.frx":0014
         TabIndex        =   0
         Top             =   120
         Width           =   9135
      End
      ...
      Begin VB.Label lblPeriod 
         ...
         BackColor       =   &H00C0C0C0&
         BackStyle       =   0  'Transparent
         Caption         =   "Annual Entitlement Period"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin Threed.SSPanel panControls 
      ...
      Begin VB.CommandButton cmdPrintAll 
         ...
         Left            =   240
         TabIndex        =   181
         Tag             =   "Print all Annual Entitlement Report"
         Top             =   120
         Width           =   1215
      End
   ...
   End
End
Attribute VB_Name = "frmSVacAnnEnt"
...
If Len(dlpDateRange(0).Text) > 0 Then
  If Not IsDate(dlpDateRange(0).Text) Then
    MsgBox "Invalid Annual Entitlement Period From Date"
...
If Len(dlpDateRange(1).Text) > 0 Then
  If Not IsDate(dlpDateRange(1).Text) Then
    MsgBox "Invalid Annual Entitlement Period To Date"
...
gdbAdoIhr001.BeginTrans
    ...
    If if_Entitle Then
        ...
        snapEntitle("ED_ANNVAC") = dblEntitleUpd    ''VitalAire - Ticket #13979 - Annual Entitlement
...
Msg = Msg & Chr(10) & "The Annual Entitlement Rules?  "

a% = MsgBox(Msg, 36, "Confirm Delete")
If a% <> 6 Then Exit Sub

Call getWSQLQ("C")
SQLQ = "DELETE FROM HRANNVACENT WHERE " & fglbVSQLQ
...
Else
    Call getWSQLQ("C")
    SQLQ = "SELECT * FROM HRANNVACENT WHERE " & fglbVSQLQ
...
Me.vbxCrystal.Reset
Me.vbxCrystal.WindowTitle = "Annual Entitlement Master Report"
...
SQLQ = "(1=1) "
If Len(clpDiv.Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_DIV} = '" & clpDiv.Text & "'"
If Len(clpDept.Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_DEPT} = '" & clpDept.Text & "'"
If Len(clpCode(0).Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_ORG} = '" & clpCode(0).Text & "'"
If Len(dlpAsOf.Text) > 0 Then
    dtYYY% = Year(dlpAsOf.Text)
    dtMM% = month(dlpAsOf.Text)
    dtDD% = Day(dlpAsOf.Text)
    SQLQ = SQLQ & " AND {HRANNVACENT.VE_EDATE} = Date(" & dtYYY% & ", " & dtMM% & ", " & dtDD% & ") "
End If
If Len(clpCode(1).Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_EMP} = '" & clpCode(1).Text & "'"
If Len(clpPT.Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_PT} = '" & clpPT.Text & "' "
If glbCompSerial = "S/N - 2380W" Then 'VitalAire Ticket #12591
    If Len(medHours.Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_GRPCD} = '" & medHours.Text & "'"
Else
    If Len(clpCode(2).Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_GRPCD} = '" & clpCode(2).Text & "'"
End If
If Len(clpCode(3).Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_SECTION} = '" & clpCode(3).Text & "'"
If Len(clpCode(4).Text) > 0 Then SQLQ = SQLQ & " AND {HRANNVACENT.VE_LOC} = '" & clpCode(4).Text & "'"
If Len(dlpDateRange(0).Text) > 0 Then
    dtYYY% = Year(dlpDateRange(0).Text)
    dtMM% = month(dlpDateRange(0).Text)
    dtDD% = Day(dlpDateRange(0).Text)
    SQLQ = SQLQ & " AND {HRANNVACENT.VE_FRDATE}  = Date(" & dtYYY% & ", " & dtMM% & ", " & dtDD% & ") "
End If
If Len(dlpDateRange(1).Text) > 0 Then
    dtYYY% = Year(dlpDateRange(1).Text)
    dtMM% = month(dlpDateRange(1).Text)
    dtDD% = Day(dlpDateRange(1).Text)
    SQLQ = SQLQ & " AND {HRANNVACENT.VE_TODATE}  = Date(" & dtYYY% & ", " & dtMM% & ", " & dtDD% & ") "
End If
...
MsgBox "Update Completed Successfully", vbInformation + vbOKOnly, "Annual Entitlements"
...
SQLQ = "SELECT ED_EMPNBR,ED_VACPC,ED_PVAC,ED_VAC,ED_PSICK,ED_SICK,ED_ANNVAC, ED_ANNSICK, ED_EFDATE,ED_ETDATE,"
SQLQ = SQLQ & " ED_DIV,ED_PT, ED_SECTION,ED_SALDIST, ED_LOC, ED_ORG, ED_EMP,"
SQLQ = SQLQ & " ED_DOH, ED_SENDTE,ED_UNION,ED_LTHIRE,ED_USRDAT1,ED_LUSER,ED_LDATE,ED_LTIME,ED_VACT,ED_SICKT  "
...
Automatic_VacEntitlement_Update_Run = True
    ...
    SQLQ = "SELECT DISTINCT VE_DIV,VE_DEPT,VE_ORG,VE_LOC,VE_SECTION,VE_EDATE,VE_EMP,VE_PT,VE_GRPCD,VE_FRDATE,VE_TODATE, VE_MANUAL FROM HRANNVACENT "
...
If Len(failed) = 0 Then
    MsgBox "All Rules applied", vbInformation + vbOKOnly, "Annual Entitlements"
Else
    MsgBox "The Following Rules failed:" & vbCrLf & failed, vbInformation + vbOKOnly, "Annual Entitlements"
End If
Exit Sub
...
MDIMain.panHelp(0).Caption = "An error occurred in Annual Entitlement Calculation procedure"
...
'    dblServiceYears# = (DateDiff("d", varStartDate, CVDate(xAsOf)) / 365) * 12
    ...
    If if_Entitle Then
        ...
        If fglbCompMonthly Then
            'Current Vacation Entitlement plus New Entitlement
            dblEntitleUpd# = dblEntitle# + dblNewEntitle  ' accumulate monthly values
        Else
    ...
    If if_Entitle Then
        ...
        If isLast And glbCompSerial = "S/N - 2376W" Then '#9536 on Oct 21,2005 George
            If dblDHours# <> 0 Then
                dblEntitleDays = dblEntitleUpd / dblDHours#
                dblEntitleDays = Round((dblEntitleDays / 0.25 + 0.1), 0) * 0.25 ' round to 1/4 days
                snapEntitle("ED_VAC") = dblEntitleDays * dblDHours#
            Else
                snapEntitle("ED_VAC") = dblEntitleUpd
            End If
        Else
...
Else
    ...
    Else    'Monthly or Annually
    
        If glbCompSerial = "S/N - 2335W" And InStr(1, glbSeleDiv, "HSV") = 0 Then   'Mitchelle Plastics
        ...
        Else
            ...
            'If fglbCompMonthly And Not (glbCompSerial = "S\N - 2355W" And chkManual.Value = 0) Then
            '    gdbAdoIhr001.Execute "UPDATE HREMP SET ED_ANNVAC=0"
                
                If glbCompSerial = "S\N - 2380W" Then   'VitalAire - Ticket #13979
                    'Reset Annual Entitlement field
                    gdbAdoIhr001.Execute "UPDATE HREMP SET ED_ANNVAC=0"   'ED_ANNVAC
                End If
                    ...
                    While Not snapEntitle.EOF
                        ...
                        If Not IsNull(snapEntitle("ED_EFDATE")) Then
                            ...
                            '    If Not modAnnSelection(blIsLast) Then Exit Function
                                If Not modAnnSelection(False) Then Exit Function
                            '    fglbAsOf = DateAdd("m", 1, CVDate(fglbAsOf))
