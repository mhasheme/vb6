...
Begin VB.Form frmPosResp 
   ...
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "frmPosResp.frx":0000
      Height          =   2115
      Left            =   120
      OleObjectBlob   =   "frmPosResp.frx":0014
      TabIndex        =   0
      Tag             =   "Skills Lookup"
      Top             =   600
      Width           =   9675
...
Public Sub cmdPrint_Click()
    ...
    Me.vbxCrystal.SelectionFormula = "{HRJOB_RESP.JR_JOB}='" & glbPos & "'"
    Me.vbxCrystal.Destination = crptToPrinter
    
    Me.vbxCrystal.WindowTitle = Me.Caption
    Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgPosRsp.rpt"
...
Private Sub cmdPrintAll_Click()
 '   Me.vbxCrystal.SelectionFormula = glbstrSelCri
    ...
    Me.vbxCrystal.WindowTitle = "Position Requirements"
    Me.vbxCrystal.ReportFileName = glbIHRREPORTS & "rgPosRsp.rpt"
    Me.vbxCrystal.Connect = RptODBC_SQL
    Me.vbxCrystal.Action = 1
End Sub
...
Dim rsCurrent As ADODB.Recordset
    ...
    If FromJob <> OLDJob Then
        ...
        SQLQ = "SELECT * FROM HRJOB_RESP "
        SQLQ = SQLQ & " WHERE JR_JOB = '" & FromJob & "' "
        SQLQ = SQLQ & " ORDER BY JR_SEQ"
        rsFrom.Open SQLQ, gdbAdoIhr001, adOpenForwardOnly
        Do Until rsFrom.EOF
            rsCurrent.Filter = "JR_SEQ=" & rsFrom("JR_SEQ")
            If rsCurrent.EOF Then
                rsCurrent.AddNew
                rsCurrent("JR_JOB") = OLDJob
                rsCurrent("JR_SEQ") = rsFrom("JR_SEQ")
                rsCurrent("JR_DESC") = rsFrom("JR_DESC")
                rsCurrent.Update
            End If
            rsFrom.MoveNext
        Loop
...
Me.Caption = "Position Requirements- " & lblPosition
...
Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Position Requirements", "HRJOB_RESP", "SELECT")
Call RollBack   '15June99 js
...
If IsNull(rsDATA("JR_ID")) Then
    PID& = 0
Else
    PID& = rsDATA("JR_ID") ' CLng(Val(lblID))
End If
xPosCtrl = txtSeq
...
SQLQ$ = "SELECT * FROM HRJOB_RESP "
SQLQ$ = SQLQ$ & "WHERE "
SQLQ$ = SQLQ$ & " (JR_JOB = '" & Pos$ & "' "
SQLQ$ = SQLQ$ & "AND JR_SEQ = " & xPosCtrl
SQLQ$ = SQLQ$ & "AND JR_ID <> " & ID& & ") "
...
Public Sub Display_Value()
    ...
    SQLQ = "SELECT * FROM HRJOB_RESP "
    SQLQ = SQLQ & "WHERE JR_ID = " & Data1.Recordset!JR_ID
