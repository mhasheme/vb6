...
Begin VB.Form frmRWCHrRate 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   Caption         =   "Workers Compensation Lost Work Hours Rate Report"
   ClientHeight    =   8130
   ClientLeft      =   570
   ClientTop       =   1095
   ClientWidth     =   9930
   ...
   Begin VB.Frame scrFrame 
      ...
      Begin VB.ComboBox cmbDateBased 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "fzWCHrRt.frx":0000
         Left            =   1980
         List            =   "fzWCHrRt.frx":0002
         Style           =   2  'Dropdown List
         TabIndex        =   14
      ...
      Begin VB.Label Label4 
         ...
         BackColor       =   &H80000005&
         BackStyle       =   0  'Transparent
         Caption         =   "WC Lost Work Hours Code"
         ForeColor       =   &H80000008&
         Height          =   435
         Left            =   120
         TabIndex        =   25
   ...
   End
End
Attribute VB_Name = "frmRWCHrRate"
...
'        If glbiOneWhere Then
'            If Len(dlpDateRange(2).Text) > 0 And Len(dlpDateRange(3).Text) > 0 Then
'                glbstrSelCri = glbstrSelCri & " AND {HR_ATTENDANCE.AD_DOA} >= " & Date_SQL(dlpDateRange(2)) & " AND {HR_ATTENDANCE.AD_DOA} <= " & Date_SQL(dlpDateRange(3))
'            ElseIf Len(dlpDateRange(2).Text) = 0 And Len(dlpDateRange(3).Text) > 0 Then
'                glbstrSelCri = glbstrSelCri & " AND {HR_ATTENDANCE.AD_DOA} <= " & Date_SQL(dlpDateRange(3))
'            ElseIf Len(dlpDateRange(2).Text) > 0 And Len(dlpDateRange(3).Text) = 0 Then
'                glbstrSelCri = glbstrSelCri & " AND {HR_ATTENDANCE.AD_DOA} >= " & Date_SQL(dlpDateRange(2))
'            End If
'        Else
'            If Len(dlpDateRange(2).Text) > 0 And Len(dlpDateRange(3).Text) > 0 Then
'                glbstrSelCri = " {HR_ATTENDANCE.AD_DOA} >= " & Date_SQL(dlpDateRange(2)) & " AND {HR_ATTENDANCE.AD_DOA} <= " & Date_SQL(dlpDateRange(3))
'            ElseIf Len(dlpDateRange(2).Text) = 0 And Len(dlpDateRange(3).Text) > 0 Then
'                glbstrSelCri = " {HR_ATTENDANCE.AD_DOA} <= " & Date_SQL(dlpDateRange(3))
'            ElseIf Len(dlpDateRange(2).Text) > 0 And Len(dlpDateRange(3).Text) = 0 Then
'                glbstrSelCri = " {HR_ATTENDANCE.AD_DOA} >= " & Date_SQL(dlpDateRange(2))
'            End If
'        End If
'        glbiOneWhere = True
...
If comGroup(0) <> "(none)" Then
  strRName$ = glbIHRREPORTS & "rzWCHrR1.rpt"
Else
  strRName$ = glbIHRREPORTS & "rzWCHrRt.rpt"
End If
...
Cont_Average:
    ...
    Else
        ...
        Me.vbxCrystal.DataFiles(10) = glbIHRDBW
        'Me.vbxCrystal.DataFiles(11) = glbIHRDBW
    End If
    ' window title if appropriate
    Me.vbxCrystal.WindowTitle = "Workers Compensation (WC) Lost Work Hours Rate Report"
...
Me.vbxCrystal.Formulas(2) = "Title='Workers Compensation (WC) Lost Work Hours Rate'"

'WC Incident Codes
If Len(Trim(clpCode(7).Text)) > 0 Then
    If InStr(getCodes(clpCode(7).Text), ",") <> 0 Then
        wcCode = Replace(getCodes(clpCode(7).Text), ",", " OR {HR_ATTENDANCE.AD_REASON} = ")
        If glbOracle Then
            wcCode = "if {HR_ATTENDANCE.AD_INCID}<>0 AND ({HR_ATTENDANCE.AD_REASON} = '" & wcCode & "') then {HR_ATTENDANCE.AD_HRS}"
        Else
            wcCode = "if {HR_ATTENDANCE.AD_INCID} AND ({HR_ATTENDANCE.AD_REASON} = '" & wcCode & "') then {HR_ATTENDANCE.AD_HRS}"
        End If
    Else
        If glbOracle Then
            wcCode = "if {HR_ATTENDANCE.AD_INCID}<>0 AND ({HR_ATTENDANCE.AD_REASON} = ('" & getCodes(clpCode(7).Text) & "')) then {HR_ATTENDANCE.AD_HRS}"
        Else
            wcCode = "if {HR_ATTENDANCE.AD_INCID} AND ({HR_ATTENDANCE.AD_REASON} = ('" & getCodes(clpCode(7).Text) & "')) then {HR_ATTENDANCE.AD_HRS}"
        End If
    End If
Else
    If glbOracle Then
        wcCode = "if {HR_ATTENDANCE.AD_INCID}<>0 then {HR_ATTENDANCE.AD_HRS}"
    Else
        wcCode = "if {HR_ATTENDANCE.AD_INCID} then {HR_ATTENDANCE.AD_HRS}"
    End If
End If
dscGroup$ = "WCIncHrs=" & wcCode
...
'WC Lost Time Rate
dscGroup$ = "Sum ({@WCIncHrs}, " & grpField$ & ")/{@G1TotalEmp}"
dscGroup$ = "G1WCIncRate=" & dscGroup$
