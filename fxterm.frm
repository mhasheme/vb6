VERSION 5.00
Object = "{00025600-0000-0000-C000-000000000046}#5.2#0"; "Crystl32.OCX"
Object = "{0BA686C6-F7D3-101A-993E-0000C0EF6F5E}#1.0#0"; "THREED32.OCX"
Object = "{0D6234D1-DBA2-11D1-B5DF-0060976089D0}#6.0#0"; "TODG6.OCX"
Object = "{67397AA1-7FB1-11D0-B148-00A0C922E820}#6.0#0"; "MSADODC.OCX"
Object = "{AA1F4729-68B2-4E13-A27A-B298AC8511DF}#62.0#0"; "ihrctrls.ocx"
Begin VB.Form frmTERMEMPL 
   Appearance      =   0  'Flat
   Caption         =   "List of Terminated Employees"
   ClientHeight    =   6750
   ClientLeft      =   660
   ClientTop       =   1050
   ClientWidth     =   10365
   BeginProperty Font 
      Name            =   "Arial"
      Size            =   8.25
      Charset         =   204
      Weight          =   700
      Underline       =   0   'False
      Italic          =   0   'False
      Strikethrough   =   0   'False
   EndProperty
   ForeColor       =   &H80000008&
   LinkTopic       =   "Form1"
   PaletteMode     =   1  'UseZOrder
   ScaleHeight     =   6750
   ScaleWidth      =   10365
   StartUpPosition =   2  'CenterScreen
   Begin VB.CommandButton cmdSelHeader 
      Caption         =   "Sort by Selected Header"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   375
      Left            =   5640
      TabIndex        =   4
      Top             =   5280
      Width           =   2415
   End
   Begin VB.CheckBox chkHideTranOut 
      Caption         =   "Hide Transfer Outs"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   255
      Left            =   8280
      TabIndex        =   15
      Top             =   5760
      Value           =   1  'Checked
      Visible         =   0   'False
      Width           =   1935
   End
   Begin VB.CheckBox chkHideRet 
      Caption         =   "Hide Retirees"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   255
      Left            =   8280
      TabIndex        =   14
      Top             =   5520
      Visible         =   0   'False
      Width           =   1815
   End
   Begin VB.CheckBox chkHideRehire 
      Caption         =   "Hide Rehire"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   315
      Left            =   8280
      TabIndex        =   13
      Top             =   5280
      Width           =   1515
   End
   Begin VB.CommandButton cmdPIDSort 
      Appearance      =   0  'Flat
      Caption         =   "Sort by Payroll ID"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   375
      Left            =   8160
      TabIndex        =   3
      Tag             =   "Change the sorting method of the Employee List"
      Top             =   4800
      Width           =   1935
   End
   Begin Threed.SSPanel panControls 
      Align           =   2  'Align Bottom
      Height          =   660
      Left            =   0
      TabIndex        =   8
      Top             =   6090
      Width           =   10365
      _Version        =   65536
      _ExtentX        =   18283
      _ExtentY        =   1164
      _StockProps     =   15
      ForeColor       =   0
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   204
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      BevelWidth      =   0
      BorderWidth     =   0
      BevelOuter      =   0
      Font3D          =   1
      Alignment       =   1
      Begin MSAdodcLib.Adodc Data1 
         Height          =   330
         Left            =   5760
         Top             =   120
         Visible         =   0   'False
         Width           =   1455
         _ExtentX        =   2566
         _ExtentY        =   582
         ConnectMode     =   0
         CursorLocation  =   3
         IsolationLevel  =   -1
         ConnectionTimeout=   15
         CommandTimeout  =   30
         CursorType      =   3
         LockType        =   1
         CommandType     =   8
         CursorOptions   =   0
         CacheSize       =   50
         MaxRecords      =   0
         BOFAction       =   0
         EOFAction       =   0
         ConnectStringType=   1
         Appearance      =   1
         BackColor       =   -2147483643
         ForeColor       =   -2147483640
         Orientation     =   0
         Enabled         =   -1
         Connect         =   ""
         OLEDBString     =   ""
         OLEDBFile       =   ""
         DataSourceName  =   ""
         OtherAttributes =   ""
         UserName        =   ""
         Password        =   ""
         RecordSource    =   ""
         Caption         =   "Adodc1"
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   204
            Weight          =   700
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         _Version        =   393216
      End
      Begin VB.CommandButton cmdOK 
         Appearance      =   0  'Flat
         Caption         =   "&Select"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
            Weight          =   700
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         Height          =   375
         Left            =   120
         TabIndex        =   5
         Tag             =   "Select the Employee listed above"
         Top             =   150
         Width           =   735
      End
      Begin VB.CommandButton cmdCancel 
         Appearance      =   0  'Flat
         Cancel          =   -1  'True
         Caption         =   "&Cancel"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
            Weight          =   700
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         Height          =   375
         Left            =   960
         TabIndex        =   6
         Tag             =   "Close and exit this screen"
         Top             =   150
         Width           =   795
      End
      Begin VB.CommandButton cmdPrint 
         Appearance      =   0  'Flat
         Caption         =   "&Print"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
            Weight          =   700
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         Height          =   375
         Left            =   1860
         TabIndex        =   7
         Tag             =   "Print the Employee Listing"
         Top             =   150
         Width           =   735
      End
      Begin Crystal.CrystalReport vbxCrystal 
         Left            =   5280
         Top             =   60
         _ExtentX        =   741
         _ExtentY        =   741
         _Version        =   348160
         WindowWidth     =   480
         WindowControlBox=   -1  'True
         WindowMaxButton =   -1  'True
         WindowMinButton =   -1  'True
         PrintFileType   =   2
         ReportSource    =   1
         BoundReportFooter=   -1  'True
         WindowControls  =   -1  'True
         PrintFileLinesPerPage=   60
         WindowShowPrintSetupBtn=   -1  'True
      End
   End
   Begin TrueOleDBGrid60.TDBGrid vbxTrueGrid 
      Bindings        =   "fxterm.frx":0000
      Height          =   4530
      Left            =   120
      OleObjectBlob   =   "fxterm.frx":0014
      TabIndex        =   9
      Tag             =   "Listing of Comments"
      Top             =   90
      Width           =   9975
   End
   Begin VB.TextBox txtEESearch 
      Appearance      =   0  'Flat
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   315
      Left            =   2590
      MaxLength       =   25
      TabIndex        =   0
      Tag             =   "00-Search for Surname"
      Top             =   4800
      Width           =   1935
   End
   Begin VB.CommandButton cmdEESort 
      Appearance      =   0  'Flat
      Caption         =   "Sort by Employee Number"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   375
      Left            =   5640
      TabIndex        =   2
      Tag             =   "Change the sorting method of the Employee List"
      Top             =   4800
      Width           =   2415
   End
   Begin VB.CommandButton cmdFind 
      Appearance      =   0  'Flat
      Caption         =   "&Find"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   375
      Left            =   4800
      TabIndex        =   1
      Tag             =   "Find Employee"
      Top             =   4800
      Width           =   735
   End
   Begin INFOHR_Controls.CodeLookup clpCode 
      Height          =   285
      Index           =   0
      Left            =   2280
      TabIndex        =   11
      Tag             =   "00-Administered By"
      Top             =   5280
      Width           =   3240
      _ExtentX        =   5715
      _ExtentY        =   503
      ShowUnassigned  =   1
      TABLName        =   "EDAB"
      MaxLength       =   10
   End
   Begin VB.Label lblOrderby 
      Appearance      =   0  'Flat
      AutoSize        =   -1  'True
      BackColor       =   &H80000005&
      BackStyle       =   0  'Transparent
      Caption         =   "Administered By Filter"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   195
      Left            =   240
      TabIndex        =   12
      Top             =   5325
      Width           =   1845
   End
   Begin VB.Label lblSearchBy 
      Appearance      =   0  'Flat
      AutoSize        =   -1  'True
      BackColor       =   &H80000005&
      BackStyle       =   0  'Transparent
      Caption         =   "Search by Surname"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H80000008&
      Height          =   195
      Left            =   240
      TabIndex        =   10
      Top             =   4860
      Width           =   1665
   End
End
Attribute VB_Name = "frmTERMEMPL"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim EESNameSort As Integer
Dim EEPIDSort As Boolean
Dim SortByFieldName, xFlagSortBySel As Boolean

Private Sub chkHideRehire_Click()
    Call EEList
End Sub

Private Sub chkHideRet_Click()
    Call EEList
End Sub

Private Sub chkHideTranOut_Click()
    Call EEList 'Ticket #22798
End Sub

Private Sub clpCode_Change(Index As Integer)
    If Index = 0 Then
        If Not clpCode(0).Caption = "Unassigned" Then
            Call EEList
        End If
    End If
End Sub

Private Sub clpCode_KeyUp(Index As Integer, KeyCode As Integer, Shift As Integer)
    If Index = 0 Then
        If Not clpCode(0).Caption = "Unassigned" Then
            Call EEList
        End If
    End If
End Sub

Private Sub cmdCancel_Click()
glbTermCancel = True
Unload Me
End Sub

Private Sub cmdCancel_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Sub cmdEESort_Click()
Dim xStr

txtEESearch.Text = ""
Screen.MousePointer = HOURGLASS
MDIMain.panHelp(0).Caption = "Refreshing List of Terminated Employees"
MDIMain.panHelp(1).Caption = " "
MDIMain.panHelp(2).Caption = " "

xFlagSortBySel = False
EEPIDSort = False

If EESNameSort = True Then  ' was sorted by surname
    EESNameSort = False
    lblSearchBy.Caption = "Search by Emp. #"
    cmdEESort.Caption = "Sort by Surname"
Else
    EESNameSort = True
    lblSearchBy.Caption = "Search by Surname"
    cmdEESort.Caption = "Sort by Employee Number"
End If

If EEList() = 0 Then     ' get the info for this person
    Exit Sub
End If          ' dpartment specific and populate the list

Screen.MousePointer = DEFAULT
MDIMain.panHelp(0).Caption = " "
'txtEESearch.SetFocus

End Sub

Private Sub cmdEESort_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Sub cmdFind_Click()
Dim Sch As String, SQLQ As String
Dim bkmark
Dim rsEmpNum As New ADODB.Recordset, xStr, xSelect

On Error GoTo Srch_Err
Data1.Refresh
If Not Data1.Recordset.EOF Then
    Sch = Replace(txtEESearch, "'", "''")
    If Not xFlagSortBySel Then
        If EEPIDSort Then
            SQLQ = "ED_PAYROLL_ID  >= '" & Sch & "'"
        Else
            If EESNameSort = True Then
                If glbOracle Or glbSQL Then
                    SQLQ = "UPPER(ED_SURNAME)  >= '" & UCase(Sch) & "'"
                Else
                    SQLQ = "ED_SURNAME  >= '" & Sch & "'"
                End If
                'Find this Employee# and then to search it. In order to find the "O'R" as Surname problem #7994
                'xStr = "SELECT ED_EMPNBR,TERM_SEQ,ED_SURNAME,ED_FNAME FROM Term_HREMP WHERE " & glbSeleDeptUn & " AND " & SQLQ
                'Ticket #23375 Franks 03/19/2013 - begin
                xStr = "SELECT ED_EMPNBR,Term_HRTRMEMP.TERM_SEQ,ED_SURNAME,ED_FNAME,ED_DIV "
                xStr = xStr & "FROM Term_HREMP INNER JOIN "
                xStr = xStr & "Term_HRTRMEMP ON Term_HRTRMEMP.TERM_SEQ = Term_HREMP.TERM_SEQ "
                xStr = xStr & " WHERE " & glbSeleDeptUn & " AND " & SQLQ & " "
                If chkHideRehire.Value Then 'Ticket #19286 Frank 11/09/2010
                    xStr = xStr & " AND (Term_DOR IS NULL) "
                End If
                If chkHideRet.Value Then
                    xStr = xStr & "AND NOT (ED_DIV = '9001' OR ED_DIV = '9002' OR ED_DIV = '9003' OR ED_DIV = '9005') "
                End If
                If chkHideTranOut.Value Then 'Ticket #22798
                    xStr = xStr & "AND NOT (Term_Reason = 'TOUT') "
                End If
                'Ticket #23375 Franks 03/19/2013 - end
                If Len(clpCode(0).Text) > 0 Then 'Ticket #18352
                    xStr = xStr & "AND ED_ADMINBY = '" & clpCode(0).Text & "' "
                End If
                If glbOracle Then
                    xStr = xStr & " ORDER BY UPPER(ED_SURNAME), UPPER(ED_FNAME),ED_EMPNBR"
                Else
                    xStr = xStr & " ORDER BY ED_SURNAME, ED_FNAME,ED_EMPNBR"
                End If
                rsEmpNum.Open xStr, gdbAdoIhr001X, adOpenStatic
                If Not rsEmpNum.EOF Then
                    xSelect = "TERM_SEQ=" & rsEmpNum("TERM_SEQ")
                Else
                    xSelect = " (1=1)"
                End If
                rsEmpNum.Close
                SQLQ = xSelect
                'Find this Employee# - End
            Else
                If Not glbLinamar And Not IsNumeric(txtEESearch) Then
                    Beep
                    MsgBox "Employee Identification must be numeric"
                    Exit Sub
                End If
                If glbLinamar Then
                    SQLQ = "EMPNBR >= '" & Sch & "'"
                Else
                    SQLQ = "ED_EMPNBR >= " & Sch
                End If
            End If
        End If
    Else
        If SortByFieldName = "Term_DOT" Then 'For Term Date Field
            If IsDate(Sch) Then
                SQLQ = SortByFieldName & " >= '" & Sch & "'"
            Else
                SQLQ = SortByFieldName & " >= '01/01/1800'"
            End If
        Else
            SQLQ = SortByFieldName & " >= '" & Sch & "'"
        End If
    End If
    Data1.Recordset.Find SQLQ
End If
If Data1.Recordset.EOF Then
    If Data1.Recordset.RecordCount > 0 Then Data1.Recordset.MoveFirst
    MsgBox "Employee not found"
End If

Screen.MousePointer = DEFAULT
cmdOK.SetFocus

Exit Sub

Srch_Err:
glbFrmCaption$ = Me.Caption
glbErrNum& = Err

Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "EEList", "Term_HRTRMEMP", "Find Next")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
End If

End Sub

Private Sub cmdFind_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Sub cmdOK_Click()
Dim X
Dim Response

On Error GoTo termerr

If glbLEE_SName = "Multi_EMP" Then
    If vbxTrueGrid.SelBookmarks.count = 0 Then
        glbLEE_FName = Data1.Recordset!EmpNbr
    Else
        If vbxTrueGrid.SelBookmarks.count > 1000 Then
            MsgBox vbxTrueGrid.SelBookmarks.count & " employees are selected" + Chr(10) + " Please make that less than 1000 employees"
            Exit Sub
        End If
        glbLEE_FName = ""
        For X = 0 To vbxTrueGrid.SelBookmarks.count - 1
            vbxTrueGrid.Bookmark = vbxTrueGrid.SelBookmarks(X)
            glbLEE_FName = glbLEE_FName & Data1.Recordset!EmpNbr & ","
        Next
        glbLEE_FName = Left(glbLEE_FName, Len(glbLEE_FName) - 1)
    End If
Else
    If glbOnTop = "FRMEREHIRE" And IsDate(Data1.Recordset("Term_DOR")) Then
        MsgBox "Record already Rehired"
        Exit Sub
    End If
    
    'Ticket #22682 - Release 8.0 - Check if this is the newest termination record of the employee. If not then
    'warn the user.
    If glbOnTop = "FRMEREHIRE" Then
        'Latest Termination record?
        If Not Latest_Termination(Data1.Recordset("Employee_Number"), Data1.Recordset("TERM_SEQ"), Data1.Recordset("Term_DOT")) Then
            Response = MsgBox("This is not the most recent Termination record of the employee for Rehire." & vbCrLf & vbCrLf & "Do you wish to proceed Rehiring with this Termination Record?", vbQuestion + vbYesNo, "Confirm")
            If Response = IDNO Then Exit Sub
        End If
    End If
    
    glbTermOK = True   'Added 98/05/09 by Andy
    glbTERM_Seq = Data1.Recordset("TERM_SEQ")
    glbTERM_ID = Data1.Recordset("Employee_Number")
    glbTermDate = Data1.Recordset("Term_DOT")
    glbEmpCountry = UCase(Data1.Recordset("ED_COUNTRY")) 'Ticket #20429 Franks 06/07/2011
    If Not IsNull(Data1.Recordset("ED_FNAME")) Then
        glbLEE_FName = Data1.Recordset("ED_FNAME")
    Else
        glbLEE_FName = "*ERROR*"
    End If
    If Not IsNull(Data1.Recordset("ED_SURNAME")) Then
        glbLEE_SName = Data1.Recordset("ED_SURNAME")
    Else
        glbLEE_SName = "*ERROR*"
    End If
    If IsNull(Data1.Recordset("ED_ORG")) Then
        glbUNIONTe = ""
    Else
        glbUNIONTe = Data1.Recordset("ED_ORG")
    End If
    
    glbTerm_FName = glbLEE_FName
    glbTerm_SName = glbLEE_SName
    
    If IsDate(Data1.Recordset("Term_DOR")) Then
        glbRehireDt = Data1.Recordset("Term_DOR")
    Else
        glbRehireDt = ""
    End If
    If glbWFC Then 'Get the glbBand
        glbBand = get_band(glbTERM_Seq)
        If IsNull(Data1.Recordset("ED_SIN")) Then 'Ticket #18566
            glbSIN = ""
        Else
            glbSIN = Data1.Recordset("ED_SIN")
        End If
    End If
    
    'Ticket #14775
    If glbLinamar Then
        glbLEE_ProdLine = Mid(Data1.Recordset("PROD_LINE"), 4) & " - " & GetTABLDesc("EDRG", Data1.Recordset("PROD_LINE")) 'Ticket #14775
    End If
End If

termerr:

glbTermCancel = False

Unload Me

Exit Sub

End Sub

Private Function get_band(EmpNo)
Dim rsTemp As New ADODB.Recordset
Dim SQLQ
    get_band = ""
    SQLQ = "SELECT SH_EMPNBR,SH_BAND FROM Term_SALARY_HISTORY WHERE SH_CURRENT <>0 AND TERM_SEQ = " & EmpNo
    rsTemp.Open SQLQ, gdbAdoIhr001, adOpenStatic
    If Not rsTemp.EOF Then
        If Not IsNull(rsTemp("SH_BAND")) Then
            get_band = rsTemp("SH_BAND")
        End If
    End If
    rsTemp.Close
End Function

Private Sub cmdOK_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Sub cmdPIDSort_Click()
Dim xStr
txtEESearch.Text = ""
Screen.MousePointer = HOURGLASS
MDIMain.panHelp(0).Caption = "Refreshing Terminated Employee List - Stand by"
MDIMain.panHelp(1).Caption = " "
MDIMain.panHelp(2).Caption = " "

xFlagSortBySel = False
EEPIDSort = True
lblSearchBy.Caption = "Search by Payroll ID"

If EEList() = 0 Then     ' get the info for this person
    Exit Sub
End If          ' dpartment specific and populate the list

Screen.MousePointer = DEFAULT
MDIMain.panHelp(0).Caption = lblSearchBy.Caption '"Search by Surname "    'laura jan 05 1998

End Sub

Private Sub cmdPrint_Click()
Dim RHeading As String

glbTermCancel = False

'Ticket #24480 - To prevent the change in Printer Setup from info:HR to change the Default Printer
'This setting has been done at the design level but the vbxCrystal.Reset is resetting it so doing it again here.
Me.vbxCrystal.WindowShowPrintSetupBtn = glbCRWPrintSetup

RHeading = "Employee Listing"
Me.vbxCrystal.WindowTitle = RHeading & " Report"
Me.vbxCrystal.BoundReportHeading = RHeading

Me.vbxCrystal.Action = 1

End Sub

Private Sub cmdPrint_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Function EEList()

Dim SQLQ As String
Dim countr   As Integer  ' EEList_Snap is definded at form level

On Error GoTo EEList_Err
EEList = False         ' if not found - no depts

'SQLQ = "SELECT Term_HRTRMEMP.*,ED_DEPTNO,"
'SQLQ = "SELECT Term_HRTRMEMP.EMPLOYEE_NUMBER, Term_HRTRMEMP.TERM_SEQ, Term_HRTRMEMP.TERM_ID, Term_HRTRMEMP.TERM_DOT, Term_HRTRMEMP.TERM_DOR, Term_HRTRMEMP.TERM_REASON, Term_HRTRMEMP.TERM_REHIRE,"
'Ticket #12618, it is case sensitive for Hastings SQL database
SQLQ = "SELECT Term_HRTRMEMP.Employee_Number, Term_HRTRMEMP.TERM_SEQ, Term_HRTRMEMP.Term_ID, Term_HRTRMEMP.Term_DOT, Term_HRTRMEMP.Term_DOR, Term_HRTRMEMP.Term_Reason, Term_HRTRMEMP.Term_Rehire,"
SQLQ = SQLQ & " ED_DEPTNO , "

If glbLinamar Then
    SQLQ = SQLQ & "ED_REGION AS PROD_LINE,"     'Ticket #14775
    SQLQ = SQLQ & "right(ED_EMPNBR,3)+'-'+ left(ED_EMPNBR,LEN(ED_EMPNBR)-3) AS EMPNBR,"
ElseIf glbOracle Then
    SQLQ = SQLQ & "ED_EMPNBR AS EMPNBR,"
Else
    'Ticket #19871 Franks 02/16/2011
    'SQLQ = SQLQ & "LTRIM(STR(ED_EMPNBR)) AS EMPNBR,"
    SQLQ = SQLQ & "ED_EMPNBR AS EMPNBR,"
End If

SQLQ = SQLQ & "ED_SURNAME, ED_FNAME,"
SQLQ = SQLQ & "ED_EMPNBR, ED_PAYROLL_ID,ED_COUNTRY,"

If glbLinamar Then SQLQ = SQLQ & "SUBSTRING(ED_HOMELINE,4,16) AS ED_HOMELINE,"
If glbLinamar Then SQLQ = SQLQ & "SUBSTRING(ED_REGION,4,16) AS ED_REGION,"

SQLQ = SQLQ & "ED_INTEL, ED_EMP, ED_DOH, ED_PT, ED_ORG,ED_ADMINBY "

If Not glbLinamar Then
    SQLQ = SQLQ & ",ED_DIV, ED_SIN "
End If

If glbLinamar Then SQLQ = SQLQ & ", DIVISION_NAME "

If glbOracle Then
    SQLQ = SQLQ & " FROM Term_HRTRMEMP,Term_HREMP  "
    SQLQ = SQLQ & " WHERE Term_HRTRMEMP.TERM_SEQ = Term_HREMP.TERM_SEQ "
    SQLQ = SQLQ & " AND " & glbSeleDeptUn
Else
    SQLQ = SQLQ & "FROM Term_HREMP INNER JOIN "
    SQLQ = SQLQ & "Term_HRTRMEMP ON Term_HRTRMEMP.TERM_SEQ = Term_HREMP.TERM_SEQ "
    If glbLinamar Then
        SQLQ = SQLQ & "LEFT JOIN HR_DIVISION  ON Term_HREMP.ED_DIV=HR_DIVISION.DIV "
    End If
    SQLQ = SQLQ & " Where " & glbSeleDeptUn
End If
If Len(clpCode(0).Text) > 0 Then 'Ticket #18352
    SQLQ = SQLQ & "AND ED_ADMINBY = '" & clpCode(0).Text & "' "
End If
If chkHideRehire.Value Then 'Ticket #19286 Frank 11/09/2010
    SQLQ = SQLQ & " AND (Term_DOR IS NULL) "
End If
If glbWFC Then 'Ticket #22482 Franks 08/27/2012
    If chkHideRet.Value Then
        SQLQ = SQLQ & "AND NOT (ED_DIV = '9001' OR ED_DIV = '9002' OR ED_DIV = '9003' OR ED_DIV = '9005') "
    End If
    If chkHideTranOut.Value Then 'Ticket #22798
        SQLQ = SQLQ & "AND NOT (Term_Reason = 'TOUT') "
    End If
End If

If Not xFlagSortBySel Then
    If EEPIDSort Then 'Frank 09/24/04 Ticket #6962 Add Sort by Payroll ID function
        SQLQ = SQLQ & " ORDER BY ED_PAYROLL_ID"
    Else
        If EESNameSort = True Then
            If glbOracle Then
                SQLQ = SQLQ & " ORDER BY UPPER(ED_SURNAME), UPPER(ED_FNAME),ED_EMPNBR"
            Else
                SQLQ = SQLQ & " ORDER BY ED_SURNAME, ED_FNAME,ED_EMPNBR"
            End If
        Else
            If glbLinamar Then
                SQLQ = SQLQ & " ORDER BY EMPNBR"
            Else
                SQLQ = SQLQ & " ORDER BY ED_EMPNBR"
            End If
        End If
    End If
    SQLQ = SQLQ & ",Term_HRTRMEMP.TERM_DOT DESC " 'Ticket #24695 Franks 11/28/2013, show latest DOT record first
Else
    'Ticket #25228 Franks 03/18/2014
    SQLQ = SQLQ & " ORDER BY " & SortByFieldName
End If

Data1.RecordSource = SQLQ
Data1.Refresh
Data1.Recordset.CacheSize = 100
Data1.Refresh
If Data1.Recordset.EOF Or Data1.Recordset.BOF Then
    cmdFind.Enabled = False
Else
    Data1.Recordset.MoveFirst
End If

EEList = True

Exit Function

EEList_Err:
glbFrmCaption$ = Me.Caption
glbErrNum& = Err

Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "EEList", "HREMP", "Select")
If gintRollBack% = False Then
    Resume Next
Else
    Unload Me
End If

End Function

Private Sub cmdSelHeader_Click() 'Ticket #25228 Franks 03/18/2014
If vbxTrueGrid.SelStartCol <> -1 Then
    Call SortByHeader(vbxTrueGrid.SelStartCol)
Else
    MsgBox "No Header Selected"
End If
End Sub

Private Sub SortByHeader(xNo) 'Ticket #25228 Franks 03/18/2014
Dim xStr, xTitle
txtEESearch.Text = ""
Screen.MousePointer = HOURGLASS
MDIMain.panHelp(0).Caption = "Refreshing Employee List - Stand by"
MDIMain.panHelp(1).Caption = " "
MDIMain.panHelp(2).Caption = " "

'EEPIDSort = True

SortByFieldName = ""
Select Case xNo
Case 0
    xTitle = "Search by Surname" '"Search by Surname"
    EESNameSort = True
    'lblSearchBy.Caption = "Search by Surname"
    cmdEESort.Caption = "Sort by Employee Number"
    glbSort = "NAME"
    xFlagSortBySel = False
Case 2
    xTitle = "Search by Emp. #" '"Sort by Employee Number"
    EESNameSort = False
    'lblSearchBy.Caption = "Search by Emp. #"
    cmdEESort.Caption = "Sort by Surname "
    glbSort = "NUMBER"
    xFlagSortBySel = False
Case 3
    xTitle = "Search by Payroll ID"
    EEPIDSort = True
    xFlagSortBySel = False
Case Else
    xTitle = "Search by " & Me.vbxTrueGrid.Columns(xNo).Caption
    SortByFieldName = Me.vbxTrueGrid.Columns(xNo).DataField
    xFlagSortBySel = True
End Select
lblSearchBy.Caption = xTitle

vbxTrueGrid.Tag = "ASC"

If EEList() = 0 Then     ' get the info for this person
    Exit Sub
End If          ' dpartment specific and populate the list

Screen.MousePointer = DEFAULT
MDIMain.panHelp(0).Caption = lblSearchBy.Caption '"Search by Surname "    'laura jan 05 1998

End Sub

Private Sub Form_Load()
Dim xStr
Data1.ConnectionString = glbAdoIHRAUDIT

Me.vbxTrueGrid.Columns(8).Visible = False   'Division
Me.vbxTrueGrid.Columns(10).Visible = False   'SIN/SSN
If Not glbLinamar Then
    Me.vbxTrueGrid.Columns(8).Visible = True    'Division
    Me.vbxTrueGrid.Columns(8).Caption = lStr("Division")
    Me.vbxTrueGrid.Columns(8).DataField = "ED_DIV"
    If gSec_Show_SIN_SSN Then
        Me.vbxTrueGrid.Columns(10).Caption = "SIN/SSN"   'SIN/SSN
        Me.vbxTrueGrid.Columns(10).DataField = "ED_SIN"
        Me.vbxTrueGrid.Columns(10).Visible = True
    End If
End If

lblOrderby.Caption = lStr("Administered By") & " Filter"    'Ticket #18352
Me.vbxTrueGrid.Columns(9).Caption = lStr("Administered By") 'Ticket #18352

vbxTrueGrid.Columns(11).Visible = glbLinamar 'Division
vbxTrueGrid.Columns(11).Caption = lStr(vbxTrueGrid.Columns(11).Caption)

If glbSort = "NUMBER" Then
    EESNameSort = True  'first sort is by surname
Else
    EESNameSort = False
End If
EEPIDSort = False

glbTermOK = False

Screen.MousePointer = HOURGLASS

MDIMain.panHelp(0).Caption = "Retrieving List of Terminated Employees"

'Ticket #20441 Franks 06/14/2011, Jerry asked to default it as unchecked
'If glbWFC Then 'Ticket #19286 Frank 11/09/2010
'    chkHideRehire.Value = 1
'End If

If glbWFC Then 'Ticket #22482 Franks 08/27/2012
    chkHideRet.Visible = True
    chkHideTranOut.Visible = True 'Ticket #22798
End If

Call cmdEESort_Click

If glbLEE_SName = "Multi_EMP" Then
    vbxTrueGrid.MultiSelect = 2
    If glbLEE_FName <> "" Then
        With Data1.Recordset
            If Not .EOF Then .MoveLast
            xStr = glbLEE_FName & ","
            Do Until .BOF
                If InStr(glbLEE_FName & ",", !EmpNbr & ",") <> 0 Then
                    xStr = Replace(xStr, !EmpNbr & ",", "")
                    vbxTrueGrid.SelBookmarks.Add vbxTrueGrid.Bookmark
                    DoEvents
                    If Trim(xStr) = "" Then Exit Do
                End If
                .MovePrevious
            Loop
        End With
    End If
End If

Call INI_Controls(Me) 'Ticket #18352

MDIMain.panHelp(0).Caption = "info:HR Main functions Locked until EE Selected"
Screen.MousePointer = DEFAULT

End Sub

Private Sub Form_Unload(Cancel As Integer)
'frmUEntitle.cmdZeroOut_Click
End Sub

Private Sub txtEESearch_GotFocus()
Call SetPanHelp(ActiveControl)
End Sub

Private Sub txtEESearch_KeyPress(KeyAscii As Integer)
    KeyAscii = Asc(UCase$(Chr$(KeyAscii)))
    If KeyAscii = 13 Then Call cmdFind_Click
End Sub

Private Sub vbxTrueGrid_DblClick()
frmFind = True
Call cmdOK_Click
End Sub

Private Sub vbxTrueGrid_HeadClick(ByVal ColIndex As Integer)
Dim SQLQ As String
    
       
        If vbxTrueGrid.Tag = "ASC" Then
            vbxTrueGrid.Tag = "DESC"
        Else
            vbxTrueGrid.Tag = "ASC"
        End If
        
        SQLQ = "SELECT Term_HRTRMEMP.*,ED_DEPTNO,"
        
        If glbLinamar Then
            SQLQ = SQLQ & "ED_REGION AS PROD_LINE,"     'Ticket #14775
            SQLQ = SQLQ & "right(ED_EMPNBR,3)+'-'+ left(ED_EMPNBR,LEN(ED_EMPNBR)-3) AS EMPNBR,"
        ElseIf glbOracle Then
            SQLQ = SQLQ & "ED_EMPNBR AS EMPNBR,"
        Else
            'Ticket #19871 Franks 02/16/2011
            'SQLQ = SQLQ & "LTRIM(STR(ED_EMPNBR)) AS EMPNBR,"
            SQLQ = SQLQ & "ED_EMPNBR AS EMPNBR,"
        End If
        
        SQLQ = SQLQ & "ED_SURNAME, ED_FNAME,"
        SQLQ = SQLQ & "ED_EMPNBR, ED_PAYROLL_ID,ED_COUNTRY,"
        
        If glbLinamar Then SQLQ = SQLQ & "SUBSTRING(ED_HOMELINE,4,16) AS ED_HOMELINE,"
        If glbLinamar Then SQLQ = SQLQ & "SUBSTRING(ED_REGION,4,16) AS ED_REGION,"
        
        SQLQ = SQLQ & "ED_INTEL, ED_EMP, ED_DOH, ED_PT, ED_ORG,ED_ADMINBY "
        
        If Not glbLinamar Then
            SQLQ = SQLQ & ",ED_DIV, ED_SIN "
        End If
        
        If glbLinamar Then SQLQ = SQLQ & ", DIVISION_NAME "
        
        'SQLQ = SQLQ & ", DIVISION_NAME "
        If glbOracle Then
            SQLQ = SQLQ & " FROM Term_HRTRMEMP,Term_HREMP  "
            SQLQ = SQLQ & " WHERE Term_HRTRMEMP.TERM_SEQ = Term_HREMP.TERM_SEQ "
            SQLQ = SQLQ & " AND " & glbSeleDeptUn
        Else
            SQLQ = SQLQ & " FROM Term_HREMP INNER JOIN "
            SQLQ = SQLQ & "Term_HRTRMEMP ON Term_HRTRMEMP.TERM_SEQ = Term_HREMP.TERM_SEQ "
            If glbLinamar Then
                SQLQ = SQLQ & "LEFT JOIN HR_DIVISION  ON Term_HREMP.ED_DIV=HR_DIVISION.DIV "
            End If
            SQLQ = SQLQ & " Where " & glbSeleDeptUn
        End If
        
        If Len(clpCode(0).Text) > 0 Then 'Ticket #18352
            SQLQ = SQLQ & "AND ED_ADMINBY = '" & clpCode(0).Text & "' "
        End If
        If chkHideRehire.Value Then 'Ticket #19286 Frank 11/09/2010
            SQLQ = SQLQ & " AND (Term_DOR IS NULL) "
        End If
        If glbWFC Then 'Ticket #22482 Franks 08/27/2012
            If chkHideRet.Value Then
                SQLQ = SQLQ & "AND NOT (ED_DIV = '9001' OR ED_DIV = '9002' OR ED_DIV = '9003' OR ED_DIV = '9005') "
            End If
            If chkHideTranOut.Value Then 'Ticket #22798
                SQLQ = SQLQ & "AND NOT (Term_Reason = 'TOUT') "
            End If
        End If
        If UCase(vbxTrueGrid.Columns(ColIndex).DataField) = "EMPNBR" Then
            If glbSQL Or glbOracle Then
                SQLQ = SQLQ & " ORDER BY " & vbxTrueGrid.Columns(ColIndex).DataField & " " & vbxTrueGrid.Tag
            Else
                SQLQ = SQLQ & " ORDER BY ED_EMPNBR " & vbxTrueGrid.Tag
            End If
        Else
            SQLQ = SQLQ & " ORDER BY " & vbxTrueGrid.Columns(ColIndex).DataField & " " & vbxTrueGrid.Tag
        End If

        Data1.RecordSource = SQLQ
        Data1.Refresh
End Sub



