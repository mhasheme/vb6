...
Begin VB.Form frmRIWantYouToKnow 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmRIWantYouToKnow"
...
Public Sub cmdPrint_Click()
    ...
    If CriCheck() Then
        If Not PrtForm("I Want You to Know... Criteria", Me) Then Exit Sub
...
Private Function Cri_SetAll()
    ...
    If intGenForm > 0 Then
        ...
        MsgBox "I Want You to Know... form(s) generated successfully.", vbOKOnly + vbInformation, "I Want You to Know Form..."
    ElseIf intGenForm = 0 Then
        'Employees not found
        MsgBox "No Employees in this selection criteria.", vbOKOnly + vbInformation, "I Want You to Know..."
    Else
        'Form generation was unsuccessful
        MsgBox "Problem generating 'I Want You to Know...' form.", vbOKOnly + vbCritical, "I Want You to Know Form..."
    End If
...
modSetCriteria_Err:
    ...
    Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Select Report Criteria", "I Want You to Know...", "Select")
    Cri_SetAll = False
...
EH:
    ...
    Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Form Resize", "I Want You to Know", "Form Resize")
    Resume exH
End Sub

Private Sub Form_Unload(Cancel As Integer)
    MDIMain.panHelp(0).Caption = "Select function from the menu."
    Set frmRIWantYouToKnow = Nothing  'carmen apr 2000
End Sub
...
Private Sub cmdWord_Click()
    ...
    '.ActiveDocument.FormFields("chkKnow").CheckBox.Value = True
    '.ActiveDocument.FormFields("dtLaw").Result = Date
    '.ActiveDocument.FormFields("dtLaw").TextInput.Clear

    'Open FullFileName For Append Lock Write As #intFileNumber
    'If I succeeded with Output, it killed the contents of the file.
    'If I used Append without the Lock Write, no error was raised.

'    Dim appWord = Word.Application
'    Set appWord = New Word.Application
'    sFileName = "c:\mydocuments\word.dot"
'    Set wrdDoc = appWord.Documents.Open(sFileName)
'    appWord.Visible = True
'    wrdDoc.SaveAs "c:\mydocuments\word.doc"


    If Len(elpEEID.Text) > 0 Then
        xEmpNo = " IN (" & getEmpnbr(elpEEID.Text) & ")"

        SQLQ = "SELECT ED_EMPNBR, ED_FNAME,ED_SURNAME, ED_DEPTNO, ED_DIV FROM HREMP "
        SQLQ = SQLQ & " WHERE ED_EMPNBR " & xEmpNo
        rsEmp.Open SQLQ, gdbAdoIhr001, adOpenKeyset, adLockOptimistic

        xlsFileTmpl = glbIHRREPORTS & IIf(Right(glbIHRREPORTS, 1) = "\", "", "\") & "1. Admin Update Template.dot"
        'Set wordDoc = wordApp.Documents.Add(App.Path & "\cotizacion.dot", False, wdNewBlankDocument)
        'Set wordDoc = wdApp.Documents.Add(xlsFileTmpl, False, wdNewBlankDocument)
        ...
        If Not rsEmp.EOF Then
            ...
            Do While Not rsEmp.EOF
                With wdApp
                    ...
                    xlsFileMat = glbIHRREPORTS & IIf(Right(glbIHRREPORTS, 1) = "\", "", "\") & "1. Admin Update_" & rsEmp("ED_EMPNBR") & ".doc"
                    'If (Dir(xlsFileMat)) <> "" Then Kill xlsFileMat
                    'Documents.Add.SaveAs FileName:=xlsFileMat(xlsFileTmpl, False, wdNewBlankDocument)

                    'FileCopy xlsFileTmpl, xlsFileMat

                    'Code to automate Word here.
                    '.Documents.Open xlsFileMat, , , False
                    Set wrdDoc = .Documents.Add(xlsFileTmpl, False)

                    'Show this instance of Word.
                    '.Visible = True

                    .Documents(wrdDoc).Activate     'Make the word doc Active

                    .ActiveDocument.FormFields("intEmpNo").Result = rsEmp("ED_EMPNBR")
                    .ActiveDocument.FormFields("strEmpName").Result = rsEmp("ED_FNAME") & " " & rsEmp("ED_SURNAME")
                    .ActiveDocument.FormFields("strDiv").Result = rsEmp("ED_DIV")
                    .ActiveDocument.FormFields("strDept").Result = rsEmp("ED_DEPTNO")
                    .ActiveDocument.FormFields("dtFrom").Result = Date
...
Private Function Generate_Word_Forms()
    ...
    SQLQ = "SELECT ED_EMPNBR, ED_FNAME, ED_SURNAME, ED_DEPTNO, ED_DIV FROM HREMP, HR_JOB_HISTORY "
    ...
    xlsFileTmpl = glbIHRREPORTS & IIf(Right(glbIHRREPORTS, 1) = "\", "", "\") & "I Want You To Know Form.dot"
    ...
    If Not rsEmp.EOF Then
        ...
        Do While Not rsEmp.EOF
            ...
            With wdApp
                ...
                xlsFileMat = txtFilePath.Text & IIf(Right(txtFilePath.Text, 1) = "\", "", "\") & "RF_" & rsEmp("ED_SURNAME") & "_" & rsEmp("ED_FNAME") & "_" & Format(dlpAsOf.Text, "mmddyy") & ".doc"
                ...
                .ActiveDocument.FormFields("dtDate").Result = Format(dlpAsOf.Text, "MMMM d, yyyy")
...
Err_Word_Forms:
    ...
    Call ERR_Hndlr(glbErrNum&, glbFrmCaption$, "Word Forms", "I Want You to Know...", "Generate Word Form")
